...
export const drag_elem_geo2 = d3.drag()
  .filter(function () {
    return current_layers[global._app.id_to_layer.get(this.parentElement.id)].draggable;
  })
  .subject(function () {
    // const layer_name = global._app.id_to_layer.get(this.parentElement.id);
    const symbol = current_layers[global._app.id_to_layer.get(this.parentElement.id)].symbol;
    const t = d3.select(this);
    if (symbol === 'rect') {
      return {
        x: t.attr('x'),
