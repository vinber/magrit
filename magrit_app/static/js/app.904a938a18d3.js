...
(function () {
  ...
  i18next.use(i18nextXHRBackend).init({
    ...
    backend: {
      loadPath: 'static/locales/{{lng}}/translation.904a938a18d3.json'
    }
  }, function (err, tr) {
    if (err) {
      throw err;
...
function getAvailablesFunctionnalities(layerName) {
  ...
  if (current_layers[layerName].type === 'Polygon' && (fields_stock.length > 0 || fields_ratio.length > 0)) {
  ...
  if (!skip_alert) {
    ...
    if (_proj === undefined) {
      ...
      swal({
        ...
        var custom_name = tryFindNameProj(_app.last_projection);
...
function createLegend_choro_horizontal(layer, field, title, subtitle) {
  ...
  var data_colors_label = current_layers[layer].colors_breaks.map(function (obj) {
    return { value: obj[0], color: obj[1] };
  }).reverse();
  var nb_class = data_colors_label;
...
var tryFindNameProj = function tryFindNameProj(proj_str) {
  ...
  if (o.length > 0) return o[0][1].name;else return undefined;
};
'use strict';
