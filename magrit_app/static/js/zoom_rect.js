...
const makeZoomRect = function () {
  ...
  function brushended() {
    ...
    if (!s) {
      ...
      let y_min = s[1][1],
        y_max = s[0][1];
      let transform = d3.zoomTransform(svg_map),
      ...
      const zoom_scale = 0.95 / Math.max((path_bounds[1][0] - path_bounds[0][0]) / w, (path_bounds[1][1] - path_bounds[0][1]) / h);
      const zoom_translate = [(w - zoom_scale * (path_bounds[1][0] + path_bounds[0][0])) / 2, (h - zoom_scale * (path_bounds[1][1] + path_bounds[0][1])) / 2];
