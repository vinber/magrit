...
function valid_join_check_display(val, prop){
  if (!val) {
    ...
    let ext_dataset_img = document.getElementById("img_data_ext");
    ext_dataset_img.setAttribute("src", "/static/img/b/jointrue.png");
...
const remove_existing_jointure = (layer_name) => {
  ...
  const data_layer = user_data[layer_name];
  const original_fields = current_layers[layer_name].original_fields;
  const field_difference = Object.getOwnPropertyNames(data_layer[0]).filter(f => !original_fields.has(f));
  const nbFields = field_difference.length;
  for (let i = 0, nbFt = data_layer.length; i < nbFt; i++) {
