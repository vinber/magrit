...
# heavy_expression <- "mymat <- matrix(runif(4444*4444), 4444);"
# expr <- parse(text=heavy_expression)
# timeout <- 500
...
eval_psock <- function(expr, envir=parent.frame(), timeout=60, opts){
  ...
  from("parallel", "sendCall")(child, eval, list(expr=parse(text=expr), envir=as.list(envir)));
...
test <- function(){
  ...
  eval_psock(svd(matrix(rnorm(n), k)), timeout=80);
}
