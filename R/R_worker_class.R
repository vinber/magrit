...
main_fuw <- function(identifiant, worker_url){
  worker <- R_Worker_fuw$new(identifiant = identifiant, worker_url = worker_url)
  worker$bind_socket()
  none <- tryCatch(
    worker$listen(),
...
trash_var = 
tryCatch(load(), error=function(e)e, warning=function(w) trash_var<-w, message=function(m) trash_var<-m)
