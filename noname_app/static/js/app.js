...
function setUpInterface(resume_project) {
    ...
    dv1.append('p').styles({ 'text-align': 'center', 'margin': '5px' }).insert('button').attrs({ 'id': 'btn_type_fields', 'class': 'i18n',
        ...
        'disabled': true }).styles({ cursor: 'pointer', disabled: 'true' }).html(i18next.t('app_page.box_type_fields.title')).on('click', function () {
    ...
    g.append("input").attrs({ type: "range", id: "form_rotate", value: 0,
        min: 0, max: 360, step: 1 }).styles({ width: "80px", margin: "0px 10px 9px 15px",
        float: "right", "vertical-align": "middle" }).on("input", function () {
        rotate_global(this.value);
    ...
    _i.insert('span').insert('img').attrs({ id: 'btn_free_draw', src: '/static/img/layout_icons/draw-01.png', class: 'layout_ft_ico i18n', 'data-i18n': '[title]app_page.layout_features_box.free_draw' }).on('click', function () {
        return add_layout_feature('free_draw');
    });
    _i.insert('span').insert('img').attrs({ id: 'btn_ellipse', src: '/static/img/layout_icons/ellipse-01.png', class: 'layout_ft_ico i18n', 'data-i18n': '[title]app_page.layout_features_box.ellipse' }).on('click', function () {
        return add_layout_feature('ellipse');
    });
    _i.insert('span').insert('img').attrs({ id: 'btn_graticule', src: '/static/img/layout_icons/graticule-01.png', class: 'layout_ft_ico i18n', 'data-i18n': '[title]app_page.layout_features_box.graticule' }).on('click', function () {
        return add_layout_feature('graticule');
    });
    _i.insert('span').insert('img').attrs({ id: 'btn_north', src: '/static/img/layout_icons/north-01.png', class: 'layout_ft_ico i18n', 'data-i18n': '[title]app_page.layout_features_box.north_arrow' }).on('click', function () {
        return add_layout_feature('north_arrow');
    });
    _i.insert('span').insert('img').attrs({ id: 'btn_scale', src: '/static/img/layout_icons/scale.png', class: 'layout_ft_ico i18n', 'data-i18n': '[title]app_page.layout_features_box.scale' }).on('click', function () {
        return add_layout_feature('scale');
    });
    _i.insert('span').insert('img').attrs({ id: 'btn_sphere', src: '/static/img/layout_icons/sphere-01.png', class: 'layout_ft_ico i18n', 'data-i18n': '[title]app_page.layout_features_box.sphere' }).on('click', function () {
        return add_layout_feature('sphere');
    });
    _i.insert('span').insert('img').attrs({ id: 'btn_symbol', src: '/static/img/layout_icons/symbols-01.png', class: 'layout_ft_ico i18n', 'data-i18n': '[title]app_page.layout_features_box.symbol' }).on('click', function () {
    ...
    var proj_select = proj_options.append("select").attr("id", "form_projection").styles({ "align": "center", "color": " white", "background-color": "#173a50", "border": "none" }).on("change", function () {
...
var button_result_type = new Map([["flow", '<img src="/static/img/type_geom/layer_flow.svg" class="ico_type" width="17" height="17" alt="flow"/>'], ["symbol", '<img src="/static/img/type_geom/layer_symbol.svg" class="ico_type" width="17" height="17" alt="symbol"/>'], ["grid", '<img src="/static/img/type_geom/layer_grid.svg" class="ico_type" width="17" height="17" alt="grid"/>'], ["propchoro", '<img src="/static/img/type_geom/layer_propchoro.svg" class="ico_type" width="17" height="17" alt="propchoro"/>'], ["typo", '<img src="/static/img/type_geom/layer_typo.svg" class="ico_type" width="17" height="17" alt="typo"/>'], ["discont", '<img src="/static/img/type_geom/layer_disc.svg" class="ico_type" width="17" height="17" alt="discont"/>'], ["cartogram", '<img src="/static/img/type_geom/layer_cartogram.svg" class="ico_type" width="17" height="17" alt="cartogram"/>'], ["label", '<img src="/static/img/type_geom/layer_label.svg" class="ico_type" width="17" height="17" alt="label"/>'], ["choro", '<img src="/static/img/type_geom/layer_choro.svg" class="ico_type" width="17" height="17" alt="choro"/>'], ["smooth", '<img src="/static/img/type_geom/layer_smooth.svg" class="ico_type" width="17" height="17" alt="smooth"/>'], ["prop", '<img src="/static/img/type_geom/layer_prop.svg" class="ico_type" width="17" height="17" alt="prop"/>']]);
    ...
    for (var lyr_name in current_layers) {
        if (current_layers[lyr_name].renderer && (current_layers[lyr_name].renderer.indexOf('PropSymbol') > -1 || current_layers[lyr_name].renderer.indexOf('TypoSymbols') > -1 || current_layers[lyr_name].renderer.indexOf('Label') > -1)) {
            ...
            if (symbol == "text") {
                ...
                map.select('#' + lyr_name).selectAll(symbol).attrs(function (d) {
                    var pt = path.centroid({ 'type': 'Point', 'coordinates': d.coords });
                    return { 'x': pt[0], 'y': pt[1] };
                });
            } else if (symbol == "image") {
                // Reproject pictograms :
                map.select('#' + lyr_name).selectAll(symbol).attrs(function (d, i) {
                ...
                map.select("#" + lyr_name).selectAll(symbol).style('display', function (d) {
...
var display_discretization = function display_discretization(layer_name, field_name, nb_class, type, options) {
    ...
    if (result_data.hasOwnProperty(layer_name)) var db_data = result_data[layer_name];else if (user_data.hasOwnProperty(layer_name)) var db_data = user_data[layer_name];else {
        var layer = svg_map.querySelector('#' + layer_name);
...
*
*/
var get_first_guess_span = function get_first_guess_span() {
...
var fields_PropSymbolChoro = {
    ...
    unfill: function unfill() {
        var field_size = document.getElementById("PropSymbolChoro_field_1"),
            field_color = document.getElementById("PropSymbolChoro_field_2");

        for (var i = field_size.childElementCount - 1; i >= 0; i--) {
            field_size.removeChild(field_size.children[i]);
        }

        for (var _i3 = field_color.childElementCount - 1; _i3 >= 0; _i3--) {
            field_color.removeChild(field_color.children[_i3]);
        }
...
var fields_Stewart = {
    ...
    unfill: function unfill() {
        var field_selec = document.getElementById("stewart_field"),
...
function render_stewart() {
    ...
    xhrequest("POST", '/compute/stewart', formToSend, true).then(function (res) {
        ...
        map.select("#" + n_layer_name).selectAll("path").styles(function (d, i) {
...
function make_prop_symbols(rendering_params, geojson_pt_layer) {
    ...
    if (symbol_type === 'circle') {
        map.append("g").attr("id", layer_to_add).attr("class", "result_layer layer").selectAll('circle').data(geojson_pt_layer.features).enter().append('circle').attrs(function (d, i) {
...
function render_categorical(layer, rendering_params) {
    ...
    layer_to_render.style("fill", function (d, i) {
        return colorsByFeature[i];
    }).style("fill-opacity", 0.9).style("stroke-opacity", 0.9).style("stroke", "black");
...
function render_choro(layer, rendering_params) {
    ...
    }).style('stroke-opacity', 1).style("stroke", "black");
    current_layers[layer].renderer = rendering_params['renderer'];
...
var fields_PropSymbolTypo = {
    ...
    unfill: function unfill() {
        var field1_selec = document.getElementById("PropSymbolTypo_field_1"),
...
var render_discont = function render_discont() {
    ...
    discont_worker.onmessage = function (e) {
        ...
        var result_layer = map.append("g").attr("id", new_layer_name).styles({ "stroke-linecap": "round", "stroke-linejoin": "round" }).attr("class", "result_layer layer");
...
var fields_PropSymbol = {
    ...
    unfill: function unfill() {
        var field_selec = document.getElementById("PropSymbol_field_1");
        unfillSelectInput(field_selec);
        section2.selectAll(".params").attr("disabled", true);
    }
};
...
function fillMenu_TypoSymbol() {
    ...
    var b = dv2.insert('p').style("margin", "auto").attr('class', 'params_section2');
...
function render_FlowMap(field_i, field_j, field_fij, name_join_field, disc_type, new_user_layer_name) {
    ...
    xhrequest("POST", '/compute/links', formToSend, true).then(function (data) {
        ...
        var layer_to_render = map.select("#" + new_layer_name).selectAll("path"),
...
function copy_layer(ref_layer, new_name, type_result) {
    ...
    var selec_src = document.getElementById(ref_layer).getElementsByTagName("path");
    var selec_dest = document.getElementById(new_name).getElementsByTagName("path");
...
function add_layout_feature(selected_feature) {
    ...
    if (selected_feature == "text_annot") {
        ...
        if (!window.default_symbols) {
            window.default_symbols = [];
            var a = prepare_available_symbols();
            ...
            var _a = box_choice_symbol(window.default_symbols).then(function (result) {
                if (result) {
                    add_single_symbol(result.split("url(")[1].substring(1).slice(0, -2));
                }
            });
...
function make_random_color(layer) {
    ...
    block.insert("span").styles({ "cursor": "pointer", "text-align": "center" }).html(i18next.t("app_page.layer_style_popup.toggle_colors")).on("click", function (d, i) {
        map.select("#" + layer).selectAll(symbol).transition().style("fill", function () {
...
function fill_categorical(layer, field_name, symbol, color_cat_map) {
    map.select("#" + layer).selectAll(symbol).transition().style("fill", function (d) {
...
function createStyleBox(layer_name) {
    ...
    if (_fields && _fields.length > 0) {
        (function () {
            var labels_section = popup.append("p");
            var input_fields = {};
...
function get_fields_name(layer_name) {
    var elem = document.getElementById(layer_name).childNodes[0];
...
var getBlurFilter = function (size) {
    ...
    return function (size) {
        var blur_filts = defs.node().getElementsByClassName("blur");
        var blur_filt_to_use = void 0;
        for (var i = 0; i < blur_filts.length; i++) {
...
function createLegend_choro(layer, field, title, subtitle) {
    ...
    if (current_layers[layer].renderer.indexOf('TypoSymbols') == -1) legend_elems.append('rect').attr("x", xpos + boxwidth).attr("y", function (d, i) {
        ...
        return last_pos;
    }).attr('width', boxwidth).attr('height', boxheight).styles(function (d) {
        return {
            "fill": d.color, "stroke": d.color
        };
    });else legend_elems.append('image').attrs(function (d, i) {
        return {
            "x": xpos + boxwidth,
...
function get_map_template() {
    ...
    for (var _i = map_config.n_layers - 1; _i > -1; --_i) {
        var layer_name = layers._groups[0][_i].id,
            nb_ft = current_layers[layer_name].n_features,
            selection = void 0;

        layers_style[_i] = new Object();
        layers_style[_i].layer_name = layer_name;
        layers_style[_i].n_features = nb_ft;

        var lgd = document.getElementsByClassName('lgdf_' + layer_name);
        if (lgd.length == 0) {
            layers_style[_i].legend = undefined;
        } else if (lgd.length >= 1) {
            layers_style[_i].legend = lgd[0].getAttribute("display") == "none" ? "not_display" : "display";
            layers_style[_i].legend_transform = [];
            for (var _i2 = 0; _i2 < lgd.length; _i2++) {
                layers_style[_i2].legend_transform.push(ldg[_i2].getAttribute("transform"));
            }
        }

        if (current_layers[layer_name]["stroke-width-const"]) layers_style[_i]["stroke-width-const"] = current_layers[layer_name]["stroke-width-const"];

        if (current_layers[layer_name].fixed_stroke != undefined) layers_style[_i].fixed_stroke = current_layers[layer_name].fixed_stroke;

        if (current_layers[layer_name].colors_breaks) layers_style[_i].colors_breaks = JSON.stringify(current_layers[layer_name].colors_breaks);

        if (current_layers[layer_name].options_disc !== undefined) layers_style[_i].options_disc = current_layers[layer_name].options_disc;

        if (current_layers[layer_name].targeted) {
            selection = map.select("#" + layer_name).selectAll("path");
            layers_style[_i].fill_opacity = selection.style("fill-opacity");
            layers_style[_i].targeted = true;
            layers_style[_i].topo_geom = String(current_layers[layer_name].key_name);
            layers_style[_i].fill_color = current_layers[layer_name].fill_color;
            layers_style[_i].fields_type = current_layers[layer_name].fields_type;
            ...
            var features = Array.prototype.map.call(svg_map.querySelector("#" + layer_name).getElementsByTagName(type_symbol), function (d) {
                return d.__data__;
            });
            layers_style[_i].symbol = type_symbol;
            layers_style[_i].rendered_field = current_layers[layer_name].rendered_field;
            if (current_layers[layer_name].rendered_field2) layers_style[_i].rendered_field2 = current_layers[layer_name].rendered_field2;
            layers_style[_i].renderer = current_layers[layer_name].renderer;
            layers_style[_i].size = current_layers[layer_name].size;
            layers_style[_i].fill_color = current_layers[layer_name].fill_color;
            layers_style[_i].ref_layer_name = current_layers[layer_name].ref_layer_name;
            layers_style[_i].geo_pt = {
                type: "FeatureCollection",
                features: features
            };
            if (current_layers[layer_name].renderer === "PropSymbolsTypo") {
                layers_style[_i].color_map = [].concat(_toConsumableArray(current_layers[layer_name].color_map));
            }
        } else if (current_layers[layer_name].renderer == "Stewart" || current_layers[layer_name].renderer == "Gridded" || current_layers[layer_name].renderer == "Choropleth" || current_layers[layer_name].renderer == "Categorical" || current_layers[layer_name].renderer == "Carto_doug" || current_layers[layer_name].renderer == "OlsonCarto") {
            (function () {
                selection = map.select("#" + layer_name).selectAll("path");
                layers_style[_i].renderer = current_layers[layer_name].renderer;
                layers_style[_i].topo_geom = String(current_layers[layer_name].key_name);
                ...
                layers_style[_i].color_by_id = color_by_id;
                if (current_layers[layer_name].renderer == "Stewart") {
                    layers_style[_i].current_palette = current_layers[layer_name].current_palette;
                }
                if (current_layers[layer_name].renderer !== "Categorical") {
                    layers_style[_i].options_disc = current_layers[layer_name].options_disc;
                } else {
                    layers_style[_i].color_map = [].concat(_toConsumableArray(current_layers[layer_name].color_map));
                }
                if (current_layers[layer_name].renderer == "OlsonCarto") {
                    layers_style[_i].scale_max = current_layers[layer_name].scale_max;
                    layers_style[_i].scale_byFeature = current_layers[layer_name].scale_byFeature;
                }
            })();
            ...
            layers_style[_i].ref_layer_name = current_layers[layer_name].ref_layer_name;
            layers_style[_i].size = current_layers[layer_name].size;
            layers_style[_i].min_display = current_layers[layer_name].min_display;
            ...
            layers_style[_i].current_state = state_to_save;
        } else if (current_layers[layer_name].renderer == "Label") {
            selection = map.select("#" + layer_name).selectAll("text");
            var _selec = document.getElementById(layer_name).getElementsByTagName('text');
            layers_style[_i].renderer = current_layers[layer_name].renderer;
            layers_style[_i].rendered_field = current_layers[layer_name].rendered_field;
            layers_style[_i].default_font = current_layers[layer_name].default_font;
            layers_style[_i].default_size = +current_layers[layer_name].default_size.slice(0, 2);
            layers_style[_i].fill_color = current_layers[layer_name].fill_color;
            var _features = [],
                current_position = [];
            ...
            layers_style[_i].data_labels = _features;
            layers_style[_i].current_position = current_position;
        } else {
            selection = map.select("#" + layer_name).selectAll("path");
        }
        layers_style[_i].stroke_opacity = selection.style("stroke-opacity");
        layers_style[_i].fill_opacity = selection.style("fill-opacity");
    }
    ...
    return Q.all(layers_style.map(function (obj) {
        ...
        for (var _i4 = 0; _i4 < layers_style.length; _i4++) {
            if (result[_i4] && result[_i4].target) {
                layers_style[_i4].topo_geom = result[_i4].target.responseText;
            }
        }
...
function apply_user_preferences(json_pref) {
    ...
    function apply_layout_lgd_elem() {
        ...
        if (map_config.layout_features) {
            ...
            if (map_config.layout_features.ellipse) {
                for (var _i7 = 0; _i7 < map_config.layout_features.ellipse.length; _i7++) {
                    var _ft2 = map_config.layout_features.ellipse[_i7];
                    // new UserEllipse()
                }
            }
    ...
    var _loop = function _loop(_i10) {
        if (g_timeout) clearTimeout(g_timeout);
        var layer_name = layers[_i10].layer_name,
            symbol = void 0,
            layer_selec = void 0;

        var fill_opacity = layers[_i10].fill_opacity,
            stroke_opacity = layers[_i10].stroke_opacity;

        if (layers[_i10].topo_geom && layer_name != "Sphere" && layer_name != "Simplified_land_polygons") {
            var tmp = { skip_alert: true };
            if (layers[_i10].targeted) {
                tmp['target_layer_on_add'] = true;
            } else if (layers[_i10].renderer) {
                tmp['func_name'] = func_name_corresp.get(layers[_i10].renderer);
                tmp['result_layer_on_add'] = true;
            }
            tmp['choosed_name'] = layer_name;
            tmp['skip_rescale'] = true;
            handle_reload_TopoJSON(layers[_i10].topo_geom, tmp).then(function (n_layer_name) {
                layer_name = n_layer_name;
                if (layers[_i10].renderer) {
                    current_layers[layer_name].renderer = layers[_i10].renderer;
                }
                if (layers[_i10].targeted && layers[_i10].fields_type) {
                    current_layers[layer_name].fields_type = layers[_i10].fields_type;
                    document.getElementById('btn_type_fields').removeAttribute('disabled');
                }
                ...
                current_layers[layer_name].rendered_field = layers[_i10].rendered_field;
                current_layers[layer_name]['stroke-width-const'] = layers[_i10]['stroke-width-const'];
                if (layers[_i10].fixed_stroke) current_layers[layer_name].fixed_stroke = layers[_i10].fixed_stroke;
                if (layers[_i10].ref_layer_name) current_layers[layer_name].ref_layer_name = layers[_i10].ref_layer_name;
                if (layers[_i10].size) current_layers[layer_name].size = layers[_i10].size;
                if (layers[_i10].colors_breaks) current_layers[layer_name].colors_breaks = JSON.parse(layers[_i10].colors_breaks);
                if (layers[_i10].fill_color) current_layers[layer_name].fill_color = layers[_i10].fill_color;
                if (layers[_i10].renderer) {
                    if (layers[_i10].renderer == "Choropleth" || layers[_i10].renderer == "Stewart" || layers[_i10].renderer == "Gridded") {
                        layer_selec.selectAll("path").style("fill", function (d, j) {
                            return layers[_i10].color_by_id[j];
                        });
                    } else if (layers[_i10].renderer == "Links") {
                        current_layers[layer_name].linksbyId = layers[_i10].linksbyId;
                        current_layers[layer_name].min_display = layers[_i10].min_display;
                        current_layers[layer_name].breaks = layers[_i10].breaks;
                    } else if (layers[_i10].renderer == "DiscLayer") {
                        current_layers[layer_name].min_display = layers[_i10].min_display;
                        current_layers[layer_name].breaks = layers[_i10].breaks;
                        layer_selec.selectAll("path").style("stroke-width", function (d, i) {
                            return current_layers[layer_name].result.get(d.id);
                        });
                    } else if (layers[_i10].renderer && layers[_i10].renderer.startsWith("Categorical")) {
                        var rendering_params = {
                            colorByFeature: layers[_i10].color_by_id,
                            color_map: new Map(layers[_i10].color_map),
                            rendered_field: layers[_i10].rendered_field,
                            renderer: "Categorical"
                        };
                        render_categorical(layer_name, rendering_params);
                    }
                    if (layers[_i10].legend == "display") {
                        handle_legend(layer_name);
                    }
                } else if (layers[_i10].fill_color.random) {
            ...
            var options = { 'fill': layer_name == "Graticule" ? "none" : layers[_i10].fill_color,
                'stroke': layers[_i10].stroke_color,
                'fill-opacity': fill_opacity,
                'stroke-opacity': stroke_opacity };
            add_layout_feature(layer_name.toLowerCase(), options);
            layer_selec = map.select('#' + layer_name);
        } else if (layer_name == "Simplified_land_polygons") {
            add_simplified_land_layer({ skip_rescale: true, 'fill': layers[_i10].fill_color, 'stroke': layers[_i10].stroke_color, 'fill_opacity': fill_opacity, 'stroke_opacity': stroke_opacity });
            layer_selec = map.select('#Simplified_land_polygons');
        } else if (layers[_i10].renderer && layers[_i10].renderer.startsWith("PropSymbol")) {
            var geojson_pt_layer = layers[_i10].geo_pt;

            // let layer_to_append = map.append("g").attr("id", layer_name).attr("class", "result_layer layer");
            var fill_color = layers[_i10].renderer == "PropSymbolsChoro" ? layers[_i10].fill_color.class : layers[_i10].fill_color;
            var rendering_params = {
                new_name: layer_name,
                field: layers[_i10].rendered_field,
                fill_color: fill_color,
                ref_value: layers[_i10].size[0],
                ref_size: layers[_i10].size[1],
                symbol: layers[_i10].symbol,
                nb_features: geojson_pt_layer.features.length,
                ref_layer_name: layers[_i10].ref_layer_name,
                renderer: layers[_i10].renderer
            };
            make_prop_symbols(rendering_params, geojson_pt_layer);
            if (layers[_i10].renderer == "PropSymbolsTypo") {
                current_layers[layer_name].color_map = new Map(layers[_i10].color_map);
            }
            if (layers[_i10].options_disc) {
                current_layers[layer_name].options_disc = layers[_i10].options_disc;
            }
            if (layers[_i10].rendered_field2) {
                current_layers[layer_name].rendered_field2 = layers[_i10].rendered_field2;
            }
            if (layers[_i10].colors_breaks) {
                current_layers[layer_name].colors_breaks = JSON.parse(layers[_i10].colors_breaks);
            }
            if (layers[_i10].legend == "display") {
                handle_legend(layer_name);
            }
        } else if (layers[_i10].renderer && layers[_i10].renderer.startsWith("Label")) {
            var _rendering_params = {
                uo_layer_name: layer_name,
                label_field: layers[_i10].rendered_field,
                color: layers[_i10].fill_color,
                ref_font_size: layers[_i10].default_size,
                font: layers[_i10].default_font
            };
            render_label(null, _rendering_params, { data: layers[_i10].data_labels });
        } else {
            null;
        }
        set_final_param();
...
function box_choice_symbol(sample_symbols, parent_css_selector) {
    ...
    box_select.selectAll("svg").attr("id", function () {
        ...
        var svg_dataUrl = ['url("data:image/svg+xml;base64,', btoa(new XMLSerializer().serializeToString(this)), '")'].join('');
    ...
    newbox.append("button").html(i18next.t("app_page.box_choice_symbol.browse")).on("click", function () {
        ...
        input.onchange = function (event) {
            ...
            reader.onloadend = function () {
                ...
                var dataUrl_res = ['url("', result, '")'].join('');
...
function make_table(layer_name) {
    var features = svg_map.getElementById(layer_name).childNodes,
        table = [];
