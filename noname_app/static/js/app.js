...
function discretize_to_size(values, type, nb_class, min_size, max_size) {
    ...
    for (let i = 0; i < breaks.length - 1; ++i) breaks_prop.push([[breaks[i], breaks[i + 1]], class_size[i]]);
...
var display_discretization = function (layer_name, field_name, nb_class, type, options) {
    ...
    var make_diverg_button = function () {
        ...
        central_color.insert("input").attrs({ "type": "checkbox", "id": "central_color_chkbx" }).on("change", function () {
            redisplay.draw();
            if (this.checked) col_div.select("#central_color_val").style("display", "");
        });
        ...
        central_color.insert("input").attrs({ type: "color", id: "central_color_val", value: "#e5e5e5" }).styles({ "margin": "0px 10px", "display": "none" }).on("change", redisplay.draw);
    };
    ...
    var make_overlay_elements = function () {
        ...
        rug_plot.selectAll('.indiv').data(values.map(i => ({ value: +i }))).enter().insert('line').attrs(d => ({ class: 'indiv', x1: x(d.value), y1: svg_h - margin.bottom - 10, x2: x(d.value), y2: svg_h - margin.bottom })).styles({ 'stroke': 'red', 'fill': 'none', 'stroke-width': 1 });
    };
    ...
    var redisplay = {
        ...
        draw: function (provided_colors) {
            ...
            for (let i = 0, len = bins.length; i < len; ++i) bins[i].color = color_array[i];
            ...
            var bar = svg_histo.selectAll(".bar").data(bins).enter().append("rect").attrs((d, i) => ({
            ...
            svg_histo.selectAll(".txt_bar").data(bins).enter().append("text").attrs((d, i) => ({
            ...
            })).styles({ "color": "black", "cursor": "default", "display": "none" }).text(d => formatCount(d.val));
    ...
    if (values.length < 500) {
        ...
        choice_histo.insert('button').attrs({ id: 'button_switch_plot', class: 'i18n button_st4', 'data-i18n': '[text]disc_box.switch_ref_histo' }).styles({ padding: '3px', 'font-size': '10px' }).html(i18next.t('disc_box.switch_ref_histo')).on('click', () => {
            let str_tr;
    ...
    [[i18next.t("disc_box.sequential"), "sequential"], [i18next.t("disc_box.diverging"), "diverging"]].forEach(el => {
...
function display_categorical_box(data_layer, layer_name, field, cats) {
    ...
    newbox.append("ul").style("padding", "unset").attr("id", "sortable_typo_name").selectAll("li").data(cats).enter().append("li").styles({ margin: "auto", "list-style": "none" }).attr("class", "typo_class").attr("id", (d, i) => ["line", i].join('_'));

    newbox.selectAll(".typo_class").append("input").styles({ width: "140px", height: "auto", display: "inline-block", "vertical-align": "middle", "margin-right": "20px" }).attrs(d => ({ class: 'typo_name', value: d.display_name, id: d.name }));

    newbox.selectAll(".typo_class").insert("p").attr("class", "color_square").style("background-color", d => d.color).styles({ width: "22px", height: "22px", margin: 'auto', 'vertical-align': 'middle',
    ...
    newbox.selectAll(".typo_class").insert("span").attrs(d => ({ class: 'typo_count_ft', 'data-count': d.nb_elem })).html(d => i18next.t("app_page.symbol_typo_box.count_feature", { nb_features: +d.nb_elem }));
...
var prepare_ref_histo = function (parent_node, serie, formatCount) {
    ...
    return function (type) {
        ...
        if (type == 'histogram') {
            ...
            var bar = svg_ref_histo.selectAll(".bar").data(data).enter().append("rect").attrs(d => ({
            ...
            var simulation = d3.forceSimulation(data).force("x", d3.forceX(d => x(d.value)).strength(1)).force("y", d3.forceY(m_height / 2).strength(2)).force("collide", d3.forceCollide(4)).stop();

            for (var i = 0; i < 75; ++i) simulation.tick();

            svg_ref_histo.append("g").style("font-size", "10px").attrs({ 'class': 'x_axis', 'transform': 'translate(0,' + m_height + ')' }).call(d3.axisBottom().scale(x).ticks(4).tickFormat(formatCount)).selectAll("text").attr("y", 4).attr("x", -4).attr("dy", ".45em").attr("transform", "rotate(-40)").style("text-anchor", "end");

            var cell = svg_ref_histo.append("g").attr("class", "cells").selectAll("g").data(d3.voronoi().extent([[0, 0], [m_width, m_height]]).x(d => d.x).y(d => d.y).polygons(data)).enter().append("g");

            cell.append("circle").attrs(d => {
                if (d) return {
                    r: data.lenght < 250 ? 2.5 : data.lenght < 500 ? 1.5 : 1, transform: 'translate(' + d.data.x + ',' + d.data.y + ')' };
            });

            cell.append("path").attr('d', d => {
...
function unfillSelectInput(select_node) {
    for (let i = select_node.childElementCount - 1; i > -1; i--) select_node.removeChild(select_node.children[i]);
}
...
var get_first_guess_span = function () {
    let bbox = _target_layer_file.bbox,
        abs = Math.abs;
    let width_km = haversine_dist([bbox[0], abs(bbox[3]) - abs(bbox[1])], [bbox[2], abs(bbox[3]) - abs(bbox[1])]),
...
function test_maxmin_resolution(cell_value) {
    let bbox = _target_layer_file.bbox,
        abs = Math.abs;
    let width_km = haversine_dist([bbox[0], abs(bbox[3]) - abs(bbox[1])], [bbox[2], abs(bbox[3]) - abs(bbox[1])]);
    let height_km = haversine_dist([abs(bbox[2]) - abs(bbox[0]), bbox[1]], [abs(bbox[2]) - abs(bbox[0]), bbox[3]]);
...
function make_discretization_icons(discr_section) {
    ...
    subsection1.insert('span').attrs({ 'data-i18n': '[html]app_page.func_options.common.discretization_choice' }).html(i18next.t("app_page.func_options.common.discretization_choice"));
...
function make_ok_button(parent, id, disabled = true) {
    let a = parent.append('p').styles({ "text-align": "right", margin: "auto" });
...
function make_min_max_tableau(values, nb_class, discontinuity_type, min_size, max_size, id_parent, breaks, callback) {
    ...
    let parent_nd = d3.select("#" + id_parent).styles({ 'margin-top': '3px', 'margin-bottom': '3px' });

    parent_nd.append("p").styles({ margin: '1px', "word-spacing": "1.8em" }).html('Min - Max - Size');
    parent_nd.append("div").selectAll("p").data(breaks).enter().append("p").style("margin", "0px").attr("class", "breaks_vals").attr("id", (d, i) => ["line", i].join('_'));
...
function fillMenu_PropSymbolChoro(layer) {
    ...
    [['app_page.func_options.common.symbol_circle', 'circle'], ['app_page.func_options.common.symbol_square', 'rect']].forEach(symb => {
    ...
    fill: function (layer) {
        ...
        ok_button.on("click", function () {
            ...
            if (rendering_params[field_color.node().value]) {
                ...
                new_layer_name = new_layer_name.length > 0 && /^\w+$/.test(new_layer_name) ? check_layer_name(new_layer_name) : check_layer_name(layer + "_PropSymbolsChoro");
...
var fields_Typo = {
    ...
    unfill: function () {
        ...
        for (let i = nb_fields - 1; i > -1; --i) field_selec.removeChild(field_selec.children[i]);
...
function fillMenu_Choropleth() {
    ...
    field_selec_section.insert("span").attrs({ class: "i18n", "data-i18n": "[html]app_age.func_options.common.field" }).html(i18next.t("app_page.func_options.common.field"));
...
function render_stewart() {
    ...
    xhrequest("POST", '/compute/stewart', formToSend, true).then(res => {
        ...
        map.select("#" + n_layer_name).selectAll("path").styles((d, i) => ({ 'fill': col_pal[nb_class - 1 - i], 'fill_opacity': 1, 'stroke-opacity': 0 }));
...
function fillMenu_Stewart() {
    ...
    let func_selec = f.insert('select').attrs({ class: 'params i18n', id: "stewart_func" });
    ...
    [['exponential', 'app_page.func_options.smooth.func_exponential'], ['pareto', 'app_page.func_options.smooth.func_pareto']].forEach(fun_name => {
            ...
            if (algo === "olson") {
                ...
                if (ref_size == "mean") {
                    ...
                    for (let i = 0; i < nb_ft; ++i) {
                        let val = d_values[i],
                            scale_area;
                        if (val > mean) scale_area = 1 + scale_max / (max_dif / abs(val - mean));else if (val == mean) scale_area = 1;else scale_area = 1 - scale_max / (max_dif / abs(mean - val));
                    ...
                    for (let i = 0; i < nb_ft; ++i) {
                        let val = d_values[i],
                            val_dif = max_dif / val,
                            scale_area;
                        if (val_dif < 1) scale_area = 1;else scale_area = 1 - scale_max / val_dif;
                        transform.push(scale_area);
                    }
                }
                let formToSend = new FormData();
                formToSend.append("json", JSON.stringify({
                    ...
                    scale_max: scale_max }));
                xhrequest("POST", '/compute/olson', formToSend, true).then(result => {
                    ...
                    map.select("#" + n_layer_name).selectAll("path").style("fill-opacity", 0.8).style("stroke", "black").style("stroke-opacity", 0.8);
                    switch_accordion_section();
                }, err => {
                    display_error_during_computation();
                    console.log(err);
                ...
                xhrequest("POST", '/compute/carto_doug', formToSend, true).then(data => {
                    let options = { result_layer_on_add: true };
                    if (new_user_layer_name.length > 0 && /^\w+$/.test(new_user_layer_name)) {
                        options["choosed_name"] = new_user_layer_name;
                    }
                    ...
                    current_layers[n_layer_name].rendered_field = field_name;
                    map.select("#" + n_layer_name).selectAll("path").style("fill", function () {
...
function fillMenu_Anamorphose() {
    ...
    let algo_selec = algo_choice.insert('select').attrs({ id: 'Anamorph_algo', class: 'params i18n' });
    ...
    let field_selec = field_choice.insert('select').attrs({ class: 'params', id: 'Anamorph_field' });

    // Options for Dougenik mode :
    let doug1 = dialog_content.append('p').attr('class', 'params_section2 opt_dougenik');
    ...
    let type_scale_olson = o1.append("select").attrs({ class: "params", id: "Anamorph_olson_scale_kind" });
    ...
    [["app_page.func_options.cartogram.olson_scale_max", "max"], ["app_page.func_options.cartogram.olson_scale_mean", "mean"]].forEach(opt_field => {
...
function make_prop_symbols(rendering_params, geojson_pt_layer) {
    ...
    if (symbol_type === 'circle') {
        map.append("g").attr("id", layer_to_add).attr("class", "result_layer layer").selectAll('circle').data(geojson_pt_layer.features).enter().append('circle').attrs((d, i) => {
        ...
        }).style("fill", d => d.properties.color).style("stroke", "black").style("stroke-width", 1 / zs);
    } else if (symbol_type === "rect") {
        map.append("g").attr("id", layer_to_add).attr("class", "result_layer layer").selectAll('circle').data(geojson_pt_layer.features).enter().append('rect').attrs((d, i) => {
...
function render_categorical(layer, rendering_params) {
    ...
    layer_to_render.style("fill", (d, i) => colorsByFeature[i]).style("fill-opacity", 0.9).style("stroke-opacity", 0.9).style("stroke", "black");
...
function render_choro(layer, rendering_params) {
    ...
    map.select("#" + layer).style("opacity", 1).style("stroke-width", 0.75 / d3.zoomTransform(svg_map).k, +"px");
    layer_to_render.style('fill-opacity', 1).style("fill", (d, i) => rendering_params['colorsByFeature'][i]).style('stroke-opacity', 1).style("stroke", "black");
    current_layers[layer].renderer = rendering_params['renderer'];
...
function fillMenu_PropSymbolTypo(layer) {
    ...
    let field1_selec = a.insert('select').attrs({ class: 'params', id: 'PropSymbolTypo_field_1' });
    ...
    let ref_size = b.insert('input').attrs({ type: 'number', class: 'params', id: 'PropSymbolTypo_ref_size',
        min: 0.1, max: 100.0, value: 60.0, step: "any" }).style("width", "50px");
    b.append('span').html(' (px)');
    ...
    let ref_value = c.insert('input').styles({ 'width': '100px', "margin-left": "10px" }).attrs({ type: 'number', class: 'params', id: 'PropSymbolTypo_ref_value' }).attrs({ min: 0.1, step: 0.1 });
    ...
    let symb_selec = d.insert('select').attrs({ 'class': 'params', 'id': 'PropSymbolTypo_symbol_type' });
...
var fields_PropSymbolTypo = {
    fill: function (layer) {
        ...
        //         if(!col_map){
        //             col_map = new Map();
        //             for(let i = 0, data_layer = user_data[layer]; i < nb_features; ++i){
        //                 let value = data_layer[i][selected_field],
        //                     ret_val = col_map.get(value);
        //                 col_map.set(value, ret_val ? [ret_val[0] + 1, [i].concat(ret_val[1])] : [1, [i]]);
        //             }
        ...
        btn_typo_class.on("click", function () {
            ...
            if (cats.length > 15) {
                swal({ title: "",
                    ...
                    display_categorical_box(user_data[layer], layer, selected_field).then(function (confirmed) {
                        if (confirmed) {
                            ok_button.attr("disabled", null);
                            self.rendering_params[selected_field] = {
                                nb_class: confirmed[0], color_map: confirmed[1], colorByFeature: confirmed[2],
...
function fillMenu_Discont() {
    ...
    let discontinuity_type = c.insert('select').attrs({ class: 'params i18n', id: 'kind_Discont' });

    [['app_page.func_options.discont.type_relative', 'rel'], ['app_page.func_options.discont.type_absolute', 'abs']].forEach(k => {
    ...
    let disc_type = e.insert('select').attrs({ 'class': 'params i18n', 'id': 'Discont_discKind' });

    [['app_page.common.equal_interval', 'equal_interval'], ['app_page.common.quantiles', 'quantiles'], ['app_page.common.Q6', 'Q6'], ['app_page.common.arithmetic_progression', 'arithmetic_progression'], ['app_page.common.jenks', 'jenks']].forEach(field => {
...
var render_discont = function () {
    ...
    discont_worker.onmessage = function (e) {
        ...
        let result_layer = map.append("g").attr("id", new_layer_name).styles({ "stroke-linecap": "round", "stroke-linejoin": "round" }).attr("class", "result_layer layer");
...
function fillMenu_PropSymbol(layer) {
    ...
    let field_selec = a.insert('select').attrs({ class: 'params', 'id': "PropSymbol_field_1" });
    ...
    let ref_size = b.insert('input').attrs({ id: 'PropSymbol_ref_size', type: 'number', class: 'params', min: 0.2, max: max_allowed_size, value: 60.0, step: 0.1 }).style("width", "50px");
    b.append('span').html(" px");
    ...
    let ref_value = c.insert('input').styles({ 'width': '100px', "margin-left": "10px" }).attrs({ id: 'PropSymbol_ref_value', type: 'number', class: "params", min: 0.1, step: 0.1 });
    ...
    let symb_selec = d.insert('select').attrs({ 'class': 'params i18n', "id": "PropSymbol_symbol" });
    ...
    let color_par = color_section.append('select').attrs({ id: 'PropSymbol_nb_colors', class: 'params' });
    ...
    let col_p = dialog_content.append("p").attr('class', 'params_section2').styles({ 'padding-top': '5px', 'margin-bottom': '-5px', 'text-align': 'center' });
...
function fillMenu_TypoSymbol() {
    ...
    let field_selec = a.insert('select').attrs({ class: "params", id: "field_Symbol" });
...
function render_TypoSymbols(rendering_params, new_name) {
    ...
    let context_menu = new ContextMenu(),
        getItems = self_parent => [{ "name": i18next.t("app_page.common.edit_style"), "action": () => {
            make_style_box_indiv_symbol(self_parent);
        } }, { "name": i18next.t("app_page.common.delete"), "action": () => {
            self_parent.style.display = "none";
        } }];

    map.append("g").attrs({ id: layer_to_add, class: "layer" }).selectAll("image").data(new_layer_data.features).enter().insert("image").attrs(d => {
...
function fillMenu_griddedMap(layer) {
    ...
    ['Blues', 'BuGn', 'BuPu', 'GnBu', 'OrRd', 'PuBu', 'PuBuGn', 'PuRd', 'RdPu', 'YlGn', 'Greens', 'Greys', 'Oranges', 'Purples', 'Reds'].forEach(color => {
        col_pal.append("option").text(color).attr("value", color);
    });

    [['app_page.func_options.grid.square', 'Square'], ['app_page.func_options.grid.diamond', 'Diamond'], ['app_page.func_options.grid.hexagon', 'Hexagon']].forEach(shape => {
...
function render_Gridded(field_n, resolution, cell_shape, color_palette, new_user_layer_name) {
    ...
    if (current_layers[layer].original_fields.has(field_n)) var_to_send[field_n] = [];else var_to_send[field_n] = user_data[layer].map(i => +i[field_n]);
    ...
    xhrequest("POST", '/compute/gridded', formToSend, true).then(data => {
        let options = { result_layer_on_add: true };
        if (new_user_layer_name.length > 0 && /^\w+$/.test(new_user_layer_name)) {
            options["choosed_name"] = new_user_layer_name;
        }
...
function fillMenu_FlowMap() {
    ...
    let disc_type = discretization_section.insert('select').attrs({ class: 'params i18n', id: "FlowMap_discKind" });
...
function render_FlowMap(field_i, field_j, field_fij, name_join_field, disc_type, new_user_layer_name) {
    ...
    xhrequest("POST", '/compute/links', formToSend, true).then(data => {
        ...
        for (let i = 0; i < nb_class; ++i) current_layers[new_layer_name].breaks.push([[user_breaks[i], user_breaks[i + 1]], sizes[i]]);

        layer_to_render.style('fill-opacity', 0).style('stroke-opacity', 0.8).style("stroke-width", (d, i) => {
...
var render_label = function (layer, rendering_params, options) {
    ...
    let new_layer_data = [];
    let layer_to_add = rendering_params.uo_layer_name && rendering_params.uo_layer_name.length > 0 ? check_layer_name(rendering_params.uo_layer_name) : check_layer_name("Labels_" + layer);
    let nb_ft;
    ...
    map.append("g").attrs({ id: layer_to_add, class: "layer result_layer" }).selectAll("text").data(new_layer_data).enter().insert("text").attrs((d, i) => {
    ...
    }).styles({ "font-size": font_size, "font-family": selected_font, fill: txt_color }).text(d => d.label).on("mouseover", function () {
        this.style.cursor = "pointer";
    }).on("mouseout", function () {
        this.style.cursor = "initial";
...
function copy_layer(ref_layer, new_name, type_result) {
    ...
    for (let i = 0; i < selec_src.length; i++) selec_dest[i].__data__ = selec_src[i].__data__;
    up_legends();
...
var type_col = function (layer_name, target, skip_if_empty_values = false) {
    ...
    if (target) {
        let res = [];
        for (let k in result) if (result[k] === target && k != "_uid") res.push(k);
        return res;
    } else return result;
};
...
var type_col2 = function (table, field, skip_if_empty_values = false) {
    ...
    for (let j = 0, len = fields.length; j < len; ++j) {
        ...
        for (let i = 0; i < deepth_test; ++i) {
            ...
            if (h[val]) dups[field] = true;else h[val] = true, tmp_type = typeof val;
    ...
    for (let j = 0, len = fields.length; j < len; ++j) {
        ...
        if (tmp[field].every(ft => ft === "stock" || ft === "empty") && tmp[field].indexOf("stock") > -1) result.push({ name: field, type: "stock", has_duplicate: has_dup });else if (tmp[field].every(ft => ft === "string" || ft === "empty") && tmp[field].indexOf("string") > -1) result.push({ name: field, type: "category", has_duplicate: has_dup });else if (tmp[field].every(ft => ft === "ratio" || ft === "stock" || ft === "empty") && tmp[field].indexOf("ratio") > -1) result.push({ name: field, type: "ratio" });else result.push({ name: field, type: "unknown", has_duplicate: has_dup });
    }
    return result;
};
...
function make_box_type_fields(layer_name) {
    ...
    if (f.length === 0) fields_type = tmp.slice();else if (tmp.length > fields_type.length) tmp.forEach(d => {
    ...
    box_select.selectAll("p").insert('span').html(d => d.name);

    box_select.selectAll('p').insert('select').style('float', 'right').selectAll('option').data(ref_type).enter().insert('option').attr('value', d => d).text(d => i18next.t('app_page.box_type_fields.' + d)).exit();
    ...
    container.querySelector(".btn_ok").onclick = function () {
        let r = [];
        Array.prototype.forEach.call(document.getElementById('fields_select').getElementsByTagName('p'), elem => {
...
var contains_empty_val = function (arr) {
    for (let i = arr.length - 1; i > -1; --i) if (arr[i] == null) return true;else if (isNaN(+arr[i])) return true;
    return false;
};
...
function prop_sizer3_e(arr, fixed_value, fixed_size, type_symbol) {
    ...
    if (type_symbol == "circle") {
        ...
        for (let i = 0; i < arr_len; ++i) res.push(sqrt(abs(arr[i]) * smax / fixed_value) / pi);
    } else {
        let smax = fixed_size * fixed_size;
        for (let i = 0; i < arr_len; ++i) res.push(sqrt(abs(arr[i]) * smax / fixed_value));
    }
    return res;
}
...
function haversine_dist(A, B) {
    ...
    let a = Math.sin(dLat / 2) * Math.sin(dLat / 2) + Math.cos(lat1 * pi_dr) * Math.cos(lat2 * pi_dr) * Math.sin(dLon / 2) * Math.sin(dLon / 2);
...
function getMaximalAvailableRectangle(legend_nodes) {
    function getMaxRect() {
        ...
        var cache = new Array(rows + 1),
            stack = [];
        for (var i = 0; i < cache.length; i++) cache[i] = 0;
    ...
    function updateCache(x, cache) {
        for (var y = 0; y < rows; y++) if (mat[x][y] == 1) cache[y]++;else cache[y] = 0;
    }
...
function click_button_add_layer() {
    ...
    input.onchange = function (event) {
        let target_layer_on_add = self.id === "input_geom" ? true : self.id === "img_in_geom" ? true : self.id === "img_data_ext" ? true : self.id === "data_ext" ? true : false;
...
function handle_upload_files(files, target_layer_on_add, elem) {
    ...
    if (!(files.length == 1)) {
        var files_to_send = [];
        Array.prototype.forEach.call(files, f => f.name.indexOf('.shp') > -1 || f.name.indexOf('.dbf') > -1 || f.name.indexOf('.shx') > -1 || f.name.indexOf('.prj') > -1 ? files_to_send.push(f) : null);
        elem.style.border = '';
        ...
        elem.style.border = '3px dashed red';
        let shp_part;
        Array.prototype.forEach.call(files, f => f.name.indexOf('.shp') > -1 || f.name.indexOf('.dbf') > -1 || f.name.indexOf('.shx') > -1 || f.name.indexOf('.prj') > -1 ? shp_part = true : null);
        if (shp_part) {
            swal({ title: i18next.t("app_page.common.error") + "!",
...
function convert_dataset(file) {
    let do_convert = () => {
        ...
        xhrequest("POST", '/convert_tabular', ajaxData, true).then(data => {
...
function handle_shapefile(files, target_layer_on_add) {
    ...
    xhrequest("POST", '/convert_to_topojson', ajaxData, true).then(data => {
...
function handle_TopoJSON_files(files, target_layer_on_add) {
    ...
    xhrequest("POST", '/cache_topojson/user', ajaxData, false).then(res => {
...
function add_dataset(readed_dataset) {
    ...
    for (let i = 0; i < cols.length; i++) {
        ...
        for (let j = 0; j < readed_dataset.length; j++) {
            if (readed_dataset[j][cols[i]].replace && !isNaN(+readed_dataset[j][cols[i]].replace(",", ".")) || !isNaN(+readed_dataset[j][cols[i]])) {
...
function add_csv_geom(file, name) {
    ...
    xhrequest("POST", '/convert_csv_geo', ajaxData, true).then(data => {
...
function add_layer_topojson(text, options) {
    ...
    map.append("g").attr("id", lyr_name_to_add).attr("class", data_to_load ? "targeted_layer layer" : "layer").styles({ "stroke-linecap": "round", "stroke-linejoin": "round" }).selectAll(".subunit").data(topojson.feature(topoObj, topoObj_objects).features).enter().append("path").attr("d", path).attr("id", function (d, ix) {
        ...
        "fill-opacity": type != 'Line' ? 0.90 : 0 }).attrs({ "height": "100%", "width": "100%" });

    let class_name = [target_layer_on_add ? "sortable_target " : result_layer_on_add ? "sortable_result " : null, lyr_name_to_add].join('');
    ...
    if (target_layer_on_add) {
        ...
        d3.select('#input_geom').attr("layer-target-tooltip", layer_tooltip_content).html(['<b>', _lyr_name_display, '</b> - <i><span style="font-size:9px;">', nb_ft, ' ', i18next.t("app_page.common.feature", { count: +nb_ft }), ' - ', nb_fields, ' ', i18next.t("app_page.common.field", { count: +nb_fields }), '</i></span>'].join(''));
...
function add_layout_feature(selected_feature, options = {}) {
    if (selected_feature == "text_annot") {
        let existing_annotation = document.getElementsByClassName("txt_annot"),
            existing_id = [],
            new_id;
        if (existing_annotation) existing_id = Array.prototype.map.call(existing_annotation, elem => +elem.childNodes[0].id.split('text_annotation_')[1]);
        for (let i = 0; i < 25; i++) {
        ...
        if (!scaleBar.displayed) {
            ...
            ask_existing_feature('scalebar').then(() => {
                scaleBar.remove();
                scaleBar.create();
            }, dismiss => {
                null;
        ...
        if (!northArrow.displayed) {
            ...
            ask_existing_feature('north_arrow').then(_ => {
                northArrow.remove();
                northArrow.display();
            }, dismiss => {
                null;
        ...
        if (!window.default_symbols) {
            ...
            a.then(confirmed => {
                let a = box_choice_symbol(window.default_symbols).then(result => {
                    if (result) {
                        add_single_symbol(result.split("url(")[1].substring(1).slice(0, -2));
                    }
                });
            });
        } else {
...
function handleCreateFreeDraw() {
    ...
    let draw_calc = map.append("g").append("rect").attrs({ class: "draw_calc", x: 0, y: 0, width: w, height: h }).style("opacity", 0.1).style("fill", "grey");
...
function add_single_symbol(symbol_dataurl, x, y, width = "30px", height = "30px") {
    let context_menu = new ContextMenu(),
        getItems = self_parent => [{ "name": "Edit style...", "action": () => {
            make_style_box_indiv_symbol(self_parent);
        } }, { "name": "Delete", "action": () => {
            self_parent.style.display = "none";
        } }];
...
function add_sample_layer() {
    ...
    tabular_datasets.forEach(layer_info => {
...
function add_simplified_land_layer(options = {}) {
    ...
    let background = map.append("g").attrs({ id: "Simplified_land_polygons", class: "layer" }).style("stroke-width", "0.3px");
...
function add_sample_geojson(name, options) {
    ...
    xhrequest("POST", '/cache_topojson/sample_data', formToSend, true).then(data => {
...
function send_remove_server(layer_name) {
    ...
    xhrequest("POST", '/layers/delete', formToSend, true).then(data => {
...
function prepare_available_symbols() {
    return xhrequest('GET', '/static/json/list_symbols.json', null).then(list_res => {
        list_res = JSON.parse(list_res);
        return Q.all(list_res.map(name => xhrequest('GET', "/static/img/svg_symbols/" + name, null))).then(symbols => {
...
function accordionize(css_selector = ".accordion", parent) {
    parent = parent && typeof parent === "object" ? parent : parent && typeof parent === "string" ? document.querySelector(parent) : document;
...
function handle_join() {
    ...
    if (!(layer_name.length === 1 && joined_dataset.length === 1)) {
        ...
        make_confirm_dialog2("dialogBox", undefined, { html_content: i18next.t("app_page.join_box.ask_forget_join") }).then(confirmed => {
            if (confirmed) {
                field_join_map = [];
        ...
        make_confirm_dialog2("dialogBox", undefined, { html_content: i18next.t("app_page.join_box.ask_diff_nb_features") }).then(confirmed => {
            if (confirmed) {
                createJoinBox(layer_name[0]);
            }
        });
...
function createJoinBox(layer) {
    ...
    for (let i = 0, len = geom_layer_fields.length; i < len; i++) button1.push(['<option value="', geom_layer_fields[i], '">', geom_layer_fields[i], '</option>'].join(''));
    button1.push("</select>");

    for (let i = 0, len = ext_dataset_fields.length; i < len; i++) if (ext_dataset_fields[i].length > 0) button2.push(['<option value="', ext_dataset_fields[i], '">', ext_dataset_fields[i], '</option>'].join(''));
    button2.push("</select>");

    let inner_box = ['<p><b><i>', i18next.t("app_page.join_box.select_fields"), '</i></b></p>', '<div style="padding:10px"><p>', i18next.t("app_page.join_box.geom_layer_field"), '</p>', button1.join(''), '<em style="float:right;">(', layer, ')</em></div>', '<div style="padding:15px 10px 10px"><p>', i18next.t("app_page.join_box.ext_dataset_field"), '<br></p>', button2.join(''), '<em style="float:right;">(', dataset_name, '.csv)</em></div>', '<br><p><strong>', i18next.t("app_page.join_box.ask_join"), '<strong></p></div>'].join('');

    make_confirm_dialog2("joinBox", i18next.t("app_page.join_box.title"), { html_content: inner_box, widthFitContent: true }).then(confirmed => {
        if (confirmed) {
...
function handle_click_layer(layer_name) {
    if (layer_name == "Graticule") createStyleBoxGraticule();else if (current_layers[layer_name].renderer && current_layers[layer_name].renderer.indexOf("PropSymbol") > -1) createStyleBox_ProbSymbol(layer_name);else if (current_layers[layer_name].renderer && current_layers[layer_name].renderer == "Label") createStyleBoxLabel(layer_name);else if (current_layers[layer_name].renderer && current_layers[layer_name].renderer == "TypoSymbols") createStyleBoxTypoSymbols(layer_name);else createStyleBox(layer_name);
    return;
};
...
function make_random_color(layer, symbol = "path") {
    ...
    block.insert("span").styles({ "cursor": "pointer", "text-align": "center" }).html(i18next.t("app_page.layer_style_popup.toggle_colors")).on("click", function (d, i) {
        map.select("#" + layer).selectAll(symbol).transition().style("fill", () => Colors.names[Colors.random()]);
...
function fill_categorical(layer, field_name, symbol, color_cat_map) {
    map.select("#" + layer).selectAll(symbol).transition().style("fill", d => color_cat_map.get(d.properties[field_name]));
}
...
function make_categorical_color_menu(fields, layer, fill_prev, symbol = "path") {
    ...
    field_selec.on("change", function () {
        ...
        for (let val of cats) color_cat_map.set(val, Colors.names[Colors.random()]);
...
let cloneObj = function (obj) {
    if (obj === null || typeof obj !== "object") return obj;else if (obj.toString() == "[object Map]") {
...
function createStyleBoxLabel(layer_name) {
    ...
    popup.append("p").style("text-align", "center").insert("button").attr("id", "reset_labels_loc").attr("class", "button_st4").text(i18next.t("app_page.layer_style_popup.reset_labels_location")).on("click", function () {
        selection.transition().attr("x", (d, i) => d.coords[0]).attr("y", (d, i) => d.coords[1]);
    });
    ...
    let choice_font = font_section.insert("select").style("float", "right").on("change", function () {
...
function createStyleBoxGraticule(layer_name) {
    ...
    opacity_choice.append("span").attr("id", "graticule_opacity_txt").style("float", "right").html(current_params.opacity);
    opacity_choice.append("input").attrs({ type: "range", value: current_params.opacity }).styles({ "width": "58px", "vertical-align": "middle", "display": "inline", "float": "right" }).on("change", function () {
...
function createStyleBox(layer_name) {
    ...
    make_confirm_dialog2("styleBox", layer_name, { top: true, widthFitContent: true, draggable: true }).then(function (confirmed) {
        if (confirmed) {
            ...
            let lgd = document.querySelector([_type_layer_links ? "#legend_root_links.lgdf_" : "#legend_root.lgdf_", layer_name].join(''));
            ...
            if (type === "Point" && !renderer) {
                ...
                if (fill_meth == "single") selection.style("stroke", fill_prev.single).style("stroke-opacity", previous_stroke_opacity);else if (fill_meth == "random") selection.style("stroke-opacity", previous_stroke_opacity).style("stroke", () => Colors.name[Colors.random()]);else if (fill_math == "class" && renderer == "Links") selection.style('stroke-opacity', (d, i) => current_layers[layer_name].linksbyId[i][0]).style("stroke", stroke_prev);
            } else {
    ...
    if (type === "Point" && !renderer) {
        ...
        pt_size.append("input").attrs({ type: "range", min: 0, max: 40, value: current_pt_size }).styles({ "width": "58px", "vertical-align": "middle", "display": "inline", "float": "right", "margin-right": "0px" }).on("change", function () {
        ...
        pt_size.append("span").attr("id", "txt_pt_radius").style("float", "right").html(current_pt_size + "");
    }
    ...
    if (type !== 'Line') {
        if (current_layers[layer_name].colors_breaks == undefined && renderer != "Categorical") {
            ...
            popup.append('p').style("margin", "auto").style("text-align", "center").append("button").attr("class", "button_disc").html(i18next.t("app_page.layer_style_popup.choose_discretization")).on("click", function () {
                display_discretization(layer_name, field_to_discretize, current_layers[layer_name].colors_breaks.length, "quantiles", current_layers[layer_name].options_disc).then(function (confirmed) {
                    if (confirmed) {
                        ...
                        selection.transition().style('fill-opacity', 0.9).style("fill", (d, i) => rendering_params.colorsByFeature[i]);
                    }
                });
            });
        } else if (renderer == "Stewart") {
            ...
            var recolor_stewart = function (coloramp_name, reversed) {
                ...
                for (let i = 0; i < nb_ft; ++i) rendering_params.breaks[i][1] = new_coloramp[i];
            ...
            let seq_color_select = color_palette_section.insert("select").attr("id", "coloramp_params").on("change", function () {
                recolor_stewart(this.value, false);
            });

            ['Blues', 'BuGn', 'BuPu', 'GnBu', 'OrRd', 'PuBu', 'PuBuGn', 'PuRd', 'RdPu', 'YlGn', 'Greens', 'Greys', 'Oranges', 'Purples', 'Reds'].forEach(name => {
                seq_color_select.append("option").text(name).attr("value", name);
            });
            seq_color_select.node().value = prev_palette.name;

            var button_reverse = popup.insert("button").styles({ "display": "inline", "margin-left": "10px" }).attrs({ "class": "button_st3", "id": "reverse_colramp" }).html(i18next.t("app_page.layer_style_popup.reverse_palette")).on("click", function () {
        ...
        popup.append("button").attr("class", "button_disc").html(i18next.t("app_page.layer_style_popup.modify_size_class")).on("click", function () {
            display_discretization_links_discont(layer_name, current_layers[layer_name].rendered_field, current_layers[layer_name].breaks.length, "user_defined").then(function (result) {
                if (result) {
                    ...
                    selection.style('fill-opacity', 0).style("stroke-width", (d, i) => {
                        return links_byId[i][2];
                    });
                }
            });
        ...
        popup.append("button").attr("class", "button_disc").html(i18next.t("app_page.layer_style_popup.choose_discretization")).on("click", function () {
            display_discretization_links_discont(layer_name, "disc_value", current_layers[layer_name].breaks.length, "user_defined").then(function (result) {
                if (result) {
                    ...
                    selection.style('fill-opacity', 0).style("stroke-width", (d, i) => sizes[serie.getClass(+d.properties.disc_value)]);
                }
            });
        });
    }
    let c_section = popup.append('p').attr("class", "line_elem");
    c_section.insert("span").html(type === 'Line' ? i18next.t("app_page.layer_style_popup.color") : i18next.t("app_page.layer_style_popup.border_color"));
        ...
        if (type_symbol === "circle") {
            ...
            for (let i = 0, len = prop_values.length; i < len; i++) {
                let old_rect_size = +selec[i].getAttribute('height'),
                    centr = [+selec[i].getAttribute("x") + old_rect_size / 2 - prop_values[i] / 2, +selec[i].getAttribute("y") + old_rect_size / 2 - prop_values[i] / 2];
    ...
    make_confirm_dialog2("styleBox", layer_name, { top: true, widthFitContent: true, draggable: true }).then(function (confirmed) {
        if (confirmed) {
            ...
            if (fill_meth == "single") {
                ...
                selection.style('fill', (d, i) => d_values[i] > prev_col_breaks ? fill_prev.two[1] : fill_prev.two[0]).style('stroke-opacity', previous_stroke_opacity).style('stroke', stroke_prev);
            } else if (fill_meth == "class") {
                selection.style('fill-opacity', opacity).style("fill", function (d, i) {
        ...
        popup.append('p').style("margin", "auto").html(i18next.t("app_page.layer_style_popup.field_symbol_color", { field: field_color })).append("button").attr("class", "button_disc").html(i18next.t("app_page.layer_style_popup.choose_discretization")).on("click", function () {
            display_discretization(layer_name, field_color, current_layers[layer_name].colors_breaks.length, "quantiles", current_layers[layer_name].options_disc).then(function (confirmed) {
                if (confirmed) {
                    ...
                    selection.style('fill-opacity', 0.9).style("fill", (d, i) => rendering_params.colorsByFeature[i]);
                }
            });
        });
        ...
        popup.append('p').insert('button').attr("class", "button_disc").html(i18next.t("app_page.layer_style_popup.choose_colors")).on("click", function () {
            display_categorical_box(result_data[layer_name], layer_name, field_color, current_layers[layer_name].color_map).then(function (confirmed) {
                if (confirmed) {
                    ...
                    selection.style('fill-opacity', 0.9).style("fill", (d, i) => rendering_params.colorByFeature[i]);
                }
            });
        });
    } else {
...
function make_style_box_indiv_label(label_node) {
    ...
    make_confirm_dialog2("styleTextAnnotation", i18next.t("app_page.func_options.label.title_box_indiv")).then(confirmed => {
        if (!confirmed) {
    ...
    let selec_fonts = box_content.append("select").on("change", function () {
...
class UserArrow {
    ...
    draw() {
        let context_menu = new ContextMenu(),
            getItems = () => [{ "name": i18next.t("app_page.common.edit_style"), "action": () => {
                this.editStyle();
            } }, { "name": i18next.t("app_page.common.up_element"), "action": () => {
                this.up_element();
            } }, { "name": i18next.t("app_page.common.down_element"), "action": () => {
                this.down_element();
            } }, { "name": i18next.t("app_page.common.delete"), "action": () => {
                this.arrow.remove();
            } }];
    ...
    editStyle() {
        ...
        s3.append("button").attr("class", "button_st4").html(i18next.t("app_page.arrow_edit_box.move_points")).on("click", function () {
            ...
            let move_pt_content = d3.select(".styleBoxArrow").select(".modal-body").append('div').attr('id', 'move_pt_content').node();
...
class Textbox {
    constructor(parent, new_id_txt_annot, position = [10, 30]) {
        ...
        let drag_txt_annot = d3.drag().subject(function () {
    ...
    editStyle() {
        ...
        let textarea = content_modif_zone.append("textarea").attr("id", "annotation_content").style("margin", "5px 0px 0px").on("keyup", function () {
...
var scaleBar = {
    create: function () {
        ...
        let getItems = () => [{ "name": i18next.t("app_page.common.edit_style"), "action": () => {
                this.editStyle();
            } }, { "name": i18next.t("app_page.common.delete"), "action": () => {
                this.remove();
            } }];
    ...
    getDist: function () {
        ...
        let mult = this.unit == "km" ? 1 : this.unit == "m" ? 1000 : this.unit == "mi" ? 0.621371 : 1;
    ...
    editStyle: function () {
        ...
        let unit_select = c.insert("select").attr('id', "scale_unit").on("change", function () {
...
var northArrow = {
    display: function () {
        ...
        let arrow_gp = map.append("g").attr("id", "north_arrow").attr("class", "legend").attr("scale", 1).attr("rotate", null).style("cursor", "all-scroll");
        ...
        let getItems = () => [{ "name": i18next.t("app_page.common.options"), "action": () => {
                this.editStyle();
            } }, { "name": i18next.t("app_page.common.delete"), "action": () => {
                this.remove();
            } }];
...
class UserEllipse {
    ...
    editStyle() {
        ...
        s3.append("button").attr("class", "button_st4").html(i18next.t("app_page.ellipse_edit_box.move_points")).on("click", function () {
            ...
            box_content.style('display', 'none');
            let tmp_start_point = map.append("rect").attr("class", "ctrl_pt").attr('id', 'pt1').attr("x", (self.pt1[0] - ellipse_elem.rx.baseVal.value) * zoom_param.k + zoom_param.x).attr("y", self.pt1[1] * zoom_param.k + zoom_param.y).attr("height", 6).attr("width", 6).style("fill", "red").style("cursor", "grab").call(d3.drag().on("drag", function () {
                let t = d3.select(this);
                t.attr("x", d3.event.x);
            ...
            let tmp_end_point = map.append("rect").attrs({ class: 'ctrl_pt', height: 6, width: 6, id: 'pt2',
...
function createLegend_discont_links(layer, field, title, subtitle, rect_fill_value) {
    ...
    var legend_elems = legend_root.selectAll('.legend').append("g").data(ref_symbols_params).enter().insert('g').attr('class', (d, i) => "legend_feature lg legend_" + i);
    ...
    legend_elems.append("rect").attr("x", xrect).attr("y", d => {
        last_pos = boxgap + last_pos + last_size;
        last_size = d.size;
        return last_pos;
    }).attr('width', 45).attr('height', d => d.size).styles({ fill: color, stroke: "rgb(0, 0, 0)", "fill-opacity": 1, "stroke-width": 0 });
    ...
    let tmp_pos;
    legend_elems.append("text").attr("x", x_text_pos).attr("y", d => {
        last_pos = boxgap + last_pos + last_size;
        last_size = d.size;
        tmp_pos = last_pos - d.size / 4;
        return tmp_pos;
    }).styles({ 'alignment-baseline': 'middle', 'font-size': '10px' }).text(d => d.value[1]);
...
function make_underlying_rect(legend_root, under_rect, fill) {
    ...
    translate = translate ? translate.split("translate(")[1].split(")")[0].split(",").map(d => +d) : [0, 0];
...
function createLegend_symbol(layer, field, title, subtitle, nested = "false", rect_fill_value, rounding_precision) {
    ...
    let non_empty = Array.prototype.filter.call(ref_symbols, (d, i) => {
    ...
    if (nested == "false") {
        if (symbol_type === "circle") {
            legend_elems.append("circle").styles({ fill: color_symb_lgd, stroke: "rgb(0, 0, 0)", "fill-opacity": 1 }).attrs((d, i) => {
            ...
            legend_elems.append("text").attrs((d, i) => {
                last_pos = i * boxgap + d.size + last_pos + last_size;
                last_size = d.size;
                return {
            ...
            }).styles({ 'alignment-baseline': 'middle', 'font-size': '10px' }).text(d => round_value(d.value, rounding_precision));
            ...
            legend_elems.append("rect").styles({ fill: color_symb_lgd, stroke: "rgb(0, 0, 0)", "fill-opacity": 1 }).attrs((d, i) => {
            ...
            legend_elems.append("text").attr("x", x_text_pos).attr("y", (d, i) => {
                last_pos = i * boxgap + d.size / 2 + last_pos + last_size;
                last_size = d.size;
        ...
        if (symbol_type === "circle") {
            legend_elems.append("circle").attrs(d => ({
                cx: xpos + space_elem + boxgap + max_size / 2,
            ...
            last_pos = y_pos2;last_size = 0;
            legend_elems.append("text").attr("x", xpos + space_elem + boxgap + max_size * 1.5 + 5).attr("y", d => ypos + 45 + max_size * 2 - max_size / 2 - d.size * 2).styles({ 'alignment-baseline': 'middle', 'font-size': '10px' }).text(d => round_value(d.value, rounding_precision));
            last_pos = ypos + 30 + max_size + max_size / 2;
        } else if (symbol_type === "rect") {
            legend_elems.append("rect").attrs(d => ({
            ...
            .styles({ fill: color_symb_lgd, stroke: "rgb(0, 0, 0)", "fill-opacity": 1 });
            last_pos = y_pos2;last_size = 0;
            legend_elems.append("text").attr("x", xpos + space_elem + boxgap + max_size * 1.5 + 5).attr("y", d => ypos + 45 + max_size - d.size).styles({ 'alignment-baseline': 'middle', 'font-size': '10px' }).text(d => round_value(d.value, rounding_precision));
            last_pos = ypos + 30 + max_size + max_size / 2;
        }
    }
...
function createLegend_choro(layer, field, title, subtitle, boxgap = 0, rect_fill_value, rounding_precision, no_data_txt) {
    ...
    if (current_layers[layer].renderer.indexOf('TypoSymbols') == -1) legend_elems.append('rect').attr("x", xpos + boxwidth).attr("y", (d, i) => {
        last_pos = y_pos2 + i * boxgap + i * boxheight;
        return last_pos;
    }).attr('width', boxwidth).attr('height', boxheight).styles(d => ({
        "fill": d.color, "stroke": d.color
    }));else legend_elems.append('image').attrs((d, i) => ({
        "x": xpos + boxwidth,
    ...
    if (current_layers[layer].renderer.indexOf('Choropleth') > -1 || current_layers[layer].renderer.indexOf('PropSymbolsChoro') > -1 || current_layers[layer].renderer.indexOf('Gridded') > -1 || current_layers[layer].renderer.indexOf('Stewart') > -1) {
        ...
        legend_elems.append('text').attr("x", xpos + boxwidth * 2 + 10).attr("y", function (d, i) {
            ...
            return tmp_pos;
        }).styles({ 'alignment-baseline': 'middle', 'font-size': '10px' }).text(d => round_value(+d.value.split(' - ')[1], rounding_precision));

        legend_root.insert('text').attr("id", "lgd_choro_min_val").attr("x", xpos + boxwidth * 2 + 10).attr("y", tmp_pos + boxheight + boxgap).styles({ 'alignment-baseline': 'middle', 'font-size': '10px' }).text(d => round_value(data_colors_label[data_colors_label.length - 1].value.split(' - ')[0], rounding_precision));
    } else legend_elems.append('text').attr("x", xpos + boxwidth * 2 + 10).attr("y", (d, i) => y_pos2 + i * boxheight + i * boxgap + boxheight * 2 / 3).styles({ 'alignment-baseline': 'middle', 'font-size': '10px' }).text(d => d.value);
...
function createlegendEditBox(legend_id, layer_name) {
    ...
    if (current_layers[layer_name].renderer != "Categorical" && current_layers[layer_name].renderer != "TypoSymbols" && !(current_layers[layer_name].renderer == "PropSymbolsTypo" && legend_id.indexOf("2"))) {
    ...
    if (legend_id === "legend_root") {
        ...
        let gap_section = box_body.insert("p");
        gap_section.append("input").style('margin-left', '0px').attrs({ id: 'style_lgd', type: 'checkbox' }).on("change", function () {
...
function move_legends() {
    let legends = [svg_map.querySelectorAll("#legend_root"), svg_map.querySelectorAll("#legend_root2"), svg_map.querySelectorAll("#legend_root_links"), svg_map.querySelectorAll('#scale_bar.legend')];
    ...
    for (let j = 0; j < 4; ++j) {
        ...
        for (let i = 0, i_len = legends_type.length; i < i_len; ++i) {
            ...
            if (legend_bbox.y + legend_bbox.height > dim_heght) {
                ...
                legends_type[i].setAttribute("transform", ["translate(", [val_x, +val_y - trans_y], ")"].join(''));
            }
        }
    }
}
...
function get_map_template() {
    ...
    for (let i = map_config.n_layers - 1; i > -1; --i) {
        ...
        if (current_layers[layer_name].targeted) {
            ...
            let features = Array.prototype.map.call(svg_map.querySelector("#" + layer_name).getElementsByTagName(type_symbol), function (d) {
                return d.__data__;
            });
        ...
        } else if (current_layers[layer_name].renderer == "Stewart" || current_layers[layer_name].renderer == "Gridded" || current_layers[layer_name].renderer == "Choropleth" || current_layers[layer_name].renderer == "Categorical" || current_layers[layer_name].renderer == "Carto_doug") {
            selection = map.select("#" + layer_name).selectAll("path");
            layers_style[i].renderer = current_layers[layer_name].renderer;
            layers_style[i].topo_geom = String(current_layers[layer_name].key_name);
            layers_style[i].fill_color = current_layers[layer_name].fill_color;
            ...
            if (current_layers[layer_name].renderer == "DiscLayer") {
                ...
                for (let j = 0; j < nb_ft; j++) layers_style[i].result[j] = nb_ft;
            } else {
                layers_style[i].topo_geom = String(current_layers[layer_name].key_name);
    ...
    return Q.all(layers_style.map(obj => {
...
function load_map_template() {
    let input_button = d3.select(document.createElement('input')).attr("type", "file").attr("name", "file").attr("enctype", "text/json").on('change', function () {
...
function apply_user_preferences(json_pref) {
    ...
    var func_name_corresp = new Map([["Links", "flow"], ["PropSymbolsChoro", "choroprop"], ["Carto_doug", "cartogram"], ["PropSymbols", "prop"], ["Stewart", "smooth"], ["Gridded", "grid"], ["DiscLayer", "discont"], ["Choropleth", "choro"], ["Categorical", "typo"]]);
            ...
            handle_reload_TopoJSON(layers[i].topo_geom, tmp).then(function (n_layer_name) {
                ...
                if (layers[i].renderer) {
                    if (layers[i].renderer == "Choropleth" || layers[i].renderer == "Stewart" || layers[i].renderer == "Gridded") {
                        layer_selec.selectAll("path").style("fill", (d, j) => layers[i].color_by_id[j]);
                        ...
                        layer_selec.selectAll("path").style("stroke-width", (d, i) => {
                    ...
                    layer_selec.selectAll(symbol).style("fill", () => Colors.names[Colors.random()]);
                }
...
var display_box_symbol_typo = function (layer, field, categories) {
    ...
    newbox.append("h3").html("");
    newbox.append("p").html(i18next.t("app_page.symbol_typo_box.field_categ", { field: field, nb_class: nb_class, nb_features: nb_features }));
    newbox.append("ul").style("padding", "unset").attr("id", "typo_categories").selectAll("li").data(cats).enter().append("li").styles({ margin: "auto", "list-style": "none" }).attr("class", "typo_class").attr("id", (d, i) => ["line", i].join('_'));

    newbox.selectAll(".typo_class").append("span").attrs({ "class": "three_dots" }).style("cursor", "grab");

    newbox.selectAll(".typo_class").append("input").styles({ width: "100px", height: "auto", display: "inline-block", "vertical-align": "middle", "margin-right": "7.5px" }).attrs(d => ({ class: 'typo_name', value: d.new_name, id: d.name }));

    newbox.selectAll(".typo_class").insert("p").attrs({ class: 'symbol_section', 'title': i18next.t('app_page.symbol_typo_box.title_click') }).style("background-image", d => d.img).styles({ width: "32px", height: "32px", margin: "0px 1px 0px 1px",
    ...
    newbox.selectAll(".typo_class").insert("span").html(d => i18next.t("app_page.symbol_typo_box.count_feature", { nb_features: d.nb_elem }));
...
function box_choice_symbol(sample_symbols, parent_css_selector) {
    ...
    box_select.selectAll("p").data(sample_symbols).enter().append("p").attrs(d => ({
        "id": "p_" + d[0].replace(".svg", ""),
        "title": d[0]
    })).html(d => d[1]).styles({ width: "32px", height: "32px",
        margin: "auto", display: "inline-block" });

    box_select.selectAll("svg").attr("id", function () {
        return this.parentElement.id.slice(2);
    }).attrs({ height: "32px", width: "32px" }).on("click", function () {
        ...
        let svg_dataUrl = ['url("data:image/svg+xml;base64,', btoa(new XMLSerializer().serializeToString(this)), '")'].join('');
        newbox.select("#current_symb").style("background-image", svg_dataUrl);
    });

    newbox.append("p").attr("display", "inline").html(i18next.t("app_page.box_choice_symbol.upload_symbol"));
    newbox.append("button").html(i18next.t("app_page.box_choice_symbol.browse")).on("click", function () {
    ...
    newbox.insert("p").attrs({ "class": "symbol_section", "id": "current_symb" }).styles({ width: "32px", height: "32px", display: "inline-block",
...
function get_fun_operator(operator) {
    let operators = new Map([["+", function (a, b) {
        return a + b;
    }], ["-", function (a, b) {
        return a - b;
    }], ["/", function (a, b) {
...
function add_field_table(table, layer_name, parent) {
    function check_name() {
        if (regexp_name.test(this.value) || this.value == "") chooses_handler.new_name = this.value;else {
        ...
        if (options.type_operation === "math_compute" && table.length > 3200) {
            ...
            if (operation == "truncate") {
                ...
                if (opt_val >= 0) {
                    ...
                    for (let i = 0; i < table.length; i++) table[i][new_name_field] = table[i][fil].substr(opt_val);
                }
    ...
    var txt_op = div2.append("p").attr("id", "txt_opt").text(""),
        val_opt = div2.append("input").attr("id", "val_opt").style("display", "none").on("change", function () {
...
var boxExplore2 = {
    display_table: function (table_name) {
        ...
        for (let i = 0, col = this.columns_names, len = col.length; i < len; ++i) this.columns_headers.push({ data: col[i], title: col[i] });
        ...
        if (this.tables.get(table_name)) {
            this.top_buttons.insert("button").attrs({ id: "add_field_button", class: "button_st3" }).html(i18next.t("app_page.explore_box.button_add_field")).on('click', () => {
                add_field_table(the_table, table_name, this);
            });
        }
        let txt_intro = ["<b>", table_name, "</b><br>", this.nb_features, " ", i18next.t("app_page.common.feature", { count: this.nb_features }), " - ", this.columns_names.length, " ", i18next.t("app_page.common.field", { count: this.columns_names.length })].join('');
    ...
    get_available_tables: function () {
        let target_layer = Object.getOwnPropertyNames(user_data),
            ...
            available = new Map();
        for (let lyr_name of target_layer) available.set(lyr_name, [i18next.t("app_page.common.target_layer"), user_data[lyr_name]]);
        if (ext_dataset) available.set(dataset_name, [i18next.t("app_page.common.ext_dataset"), joined_dataset[0]]);
        for (let lyr_name of result_layers) available.set(lyr_name, [i18next.t("app_page.common.result_layer"), result_data[lyr_name]]);
        return available;
    },
