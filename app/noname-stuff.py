...
app = Flask("noname-stuff")
app.config.from_object("noname-stuff")
...
def uploaded_file_summary(filename):
    ...
    return infos.translate(trans_rule).replace('\n','<br>')
...
def upload_file():
    ...
    if request.method == 'POST':
        ...
        if alert:
            return render_template('upload_srv.html', content_d=alert, raw_result='')
        for file in files_to_upload:
            ...
            if file and allowed_file(file.filename):
                ...
                continue
        if len(filenames) > 0:
            content = uploaded_file_summary(filenames[0])
            return render_template('upload_srv.html', content_d=content, raw_result='')
        else:
            return render_template('upload_srv.html', content_d=alert, raw_result='')
    else:
        return render_template('upload_srv.html', content_d='', raw_result='')
