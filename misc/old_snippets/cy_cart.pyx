...
cpdef interpolate_reconstruct_gdf(list list_x, list list_y, object geoserie, unsigned int xsize, unsigned int ysize, dict bbox, list shift, cell_size):
    cdef unsigned int i, len_li = len(list_x)
    cdef unsigned int n, nb_geoms = len(geoserie)
    ...
    for n in range(nb_geoms):
        geoserie[n] = transform_geom(geoserie[n], gridx, gridy, xsize, ysize, bbox, shift, cell_size)

    return geoserie

cdef object transform_geom(object geom, double **gridx, double **gridy, int xsize, int ysize, dict bbox, list shift, list cell_size,
                           Polygon=Polygon, MultiPolygon=MultiPolygon, LineString=LineString):
    """
    ...
    for it_geom in range(nb_geom):
        ...
        for it_bound in range(nb_bound):
            ...
            for k in range(l_coord_bound):
                ...
                y = ys[k]
                print("Original1: ", x, y)
                x = shift[0] / cell_size[0] + (x - bbox["xmin"]) / (bbox["xmax"] - bbox["xmin"])**2 / cell_size[0]
                y = shift[1] / cell_size[1] + (x - bbox["ymin"]) / (bbox["ymax"] - bbox["ymin"])**2 / cell_size[1]
                print("Original2: ", x, y)
                res_c = <double *>malloc(<unsigned int>2 * sizeof(double))
                res_c = transform_coords(gridx,gridy,xsize,ysize, <double> x, <double> y)
                print("Modified:", res_c[0], res_c[1])
                line_coord.append((res_c[0], res_c[1]))
                free(res_c)
...
def make_cart_from_density(layer_path, density, shift, cell_size, grid_size=(256, 256), output_geojson=True):
    list_x, listy = compute_xy2(grid_size[0], grid_size[1], density)
    gdf = GeoDataFrame.from_file(layer_path)
    bbox = {"xmin": gdf.total_bounds[0], "ymin": gdf.total_bounds[1],
            "xmax": gdf.total_bounds[2], "ymax": gdf.total_bounds[3]}
    res_geoms = interpolate_reconstruct_gdf(list_x, listy, gdf.geometry[:], grid_size[0], grid_size[1], bbox, shift, cell_size)
    gdf.geometry = res_geoms
    if output_geojson :
        return gdf.to_json()
    else:
        return gdf
