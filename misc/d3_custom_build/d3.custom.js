...
cubehelixLong(cubehelix$5(300, 0.5, 0.0), cubehelix$5(-240, 0.5, 1.0));

var warm = cubehelixLong(cubehelix$5(-100, 0.75, 0.35), cubehelix$5(80, 1.50, 0.8));

var cool = cubehelixLong(cubehelix$5(260, 0.75, 0.35), cubehelix$5(80, 1.50, 0.8));
...
var proto$2 = map$6.prototype;

Set$2.prototype = set$7.prototype = {
  constructor: Set$2,
  has: proto$2.has,
  add: function(value) {
    value += "";
...
Dispatch$2.prototype = dispatch$4.prototype = {
  ...
  on: function(typename, callback) {
    ...
    if (callback != null && typeof callback !== "function") throw new Error("invalid callback: " + callback);
    while (++i < n) {
      if (t = (typename = T[i]).type) _[t] = set$9(_[t], typename.name, callback);
      else if (callback == null) for (t in _) _[t] = set$9(_[t], typename.name, null);
    }
...
var setFrame$1 = typeof window === "object" && window.requestAnimationFrame ? window.requestAnimationFrame.bind(window) : function(f) { setTimeout(f, 17); };
...
Dispatch$3.prototype = dispatch$6.prototype = {
  ...
  on: function(typename, callback) {
    ...
    if (callback != null && typeof callback !== "function") throw new Error("invalid callback: " + callback);
    while (++i < n) {
      if (t = (typename = T[i]).type) _[t] = set$10(_[t], typename.name, callback);
      else if (callback == null) for (t in _) _[t] = set$10(_[t], typename.name, null);
    }
...
function set$10(type, name, callback) {
  for (var i = 0, n = type.length; i < n; ++i) {
    if (type[i].name === name) {
...
var drag = function() {
  var filter = defaultFilter,
      ...
      gestures = {},
      listeners = dispatch$6("start", "drag", "end"),
      active = 0,
      mousedownx,
      mousedowny,
      mousemoving,
...
Dispatch$4.prototype = dispatch$8.prototype = {
  ...
  on: function(typename, callback) {
    ...
    if (callback != null && typeof callback !== "function") throw new Error("invalid callback: " + callback);
    while (++i < n) {
      if (t = (typename = T[i]).type) _[t] = set$11(_[t], typename.name, callback);
      else if (callback == null) for (t in _) _[t] = set$11(_[t], typename.name, null);
    }
...
function set$11(type, name, callback) {
  for (var i = 0, n = type.length; i < n; ++i) {
    if (type[i].name === name) {
...
Dispatch$5.prototype = dispatch$10.prototype = {
  ...
  on: function(typename, callback) {
    ...
    if (callback != null && typeof callback !== "function") throw new Error("invalid callback: " + callback);
    while (++i < n) {
      if (t = (typename = T[i]).type) _[t] = set$12(_[t], typename.name, callback);
      else if (callback == null) for (t in _) _[t] = set$12(_[t], typename.name, null);
    }
...
function set$12(type, name, callback) {
  for (var i = 0, n = type.length; i < n; ++i) {
    if (type[i].name === name) {
...
function brush$1(dim) {
  var extent = defaultExtent$1,
      ...
      listeners = dispatch$10(brush, "start", "brush", "end"),
      handleSize = 6,
      touchending;
...
var areaStream = {
  ...
  polygonEnd: function() {
    ...
    this.lineStart = this.lineEnd = this.point = noop$7;
  },
  sphere: function() {
    areaSum.add(tau$3);
  }
...
var polygonContains = function(polygon, point) {
  ...
  for (var i = 0, n = polygon.length; i < n; ++i) {
    ...
    for (var j = 0; j < m; ++j, lambda0 = lambda1, sinPhi0 = sinPhi1, cosPhi0 = cosPhi1, point0 = point1) {
      ...
      sum$2.add(atan2$1(k * sign$$1 * sin$1(absDelta), cosPhi0 * cosPhi1 + k * cos$1(absDelta)));
...
function lengthLineEnd() {
  lengthStream.point = lengthStream.lineEnd = noop$7;
}
...
var areaStream$1 = {
  ...
  polygonEnd: function() {
    areaStream$1.lineStart = areaStream$1.lineEnd = areaStream$1.point = noop$7;
...
function armadilloRaw(phi0) {
  ...
  function forward(lambda, phi) {
    ...
    return [
      (1 + cosPhi) * sin$2(lambda),
      (sPhi0 * phi > -atan2$2(cosLambda, tanPhi0) - 1e-3 ? 0 : -sPhi0 * 10) + k + sin$2(phi) * cosPhi0 - (1 + cosPhi) * sinPhi0 * cosLambda // TODO D3 core should allow null or [NaN, NaN] to be returned.
    ];
  }
...
function augustRaw(lambda, phi) {
  var tanPhi = tan$1(phi / 2),
      k = sqrt$3(1 - tanPhi * tanPhi),
      c = 1 + k * cos$2(lambda /= 2),
      x = sin$2(lambda) * k / c,
      y = tanPhi / c,
      x2 = x * x,
      y2 = y * y;
...
augustRaw.invert = function(x, y) {
  x *= 3 / 8, y *= 3 / 8;
  if (!x && abs$2(y) > 1) return null;
  var x2 = x * x,
      y2 = y * y,
      s = 1 + x2 + y2,
      sin3Eta = sqrt$3((s - sqrt$3(s * s - 4 * y * y)) / 2),
      eta = asin$2(sin3Eta) / 3,
      xi = sin3Eta ? arcosh(abs$2(y / sin3Eta)) / 3 : arsinh(abs$2(x)) / 3,
      cosEta = cos$2(eta),
      coshXi = cosh$1(xi),
      d = coshXi * coshXi - cosEta * cosEta;
  return [
    sign$2(x) * 2 * atan2$2(sinh$1(xi) * cosEta, 0.25 - d),
    sign$2(y) * 2 * atan2$2(coshXi * sin$2(eta), 0.25 + d)
  ];
};
...
function bakerRaw(lambda, phi) {
  ...
  return phi0 < quarterPi$1
      ? [lambda, log$2(tan$1(quarterPi$1 + phi / 2))]
      : [lambda * cos$2(phi0) * (2 * sqrt2 - 1 / sin$2(phi0)), sign$2(phi) * (2 * sqrt2 * (phi0 - quarterPi$1) - log$2(tan$1(phi0 / 2)))];
}

bakerRaw.invert = function(x, y) {
  if ((y0 = abs$2(y)) < phi0$1) return [x, 2 * atan$1(exp$1(y)) - halfPi$3];
...
function berghausRaw(lobes) {
  ...
  function forward(lambda, phi) {
    ...
    if (abs$2(lambda) > halfPi$3) { // back hemisphere
      var theta = atan2$2(p[1], p[0]),
          r = sqrt$3(p[0] * p[0] + p[1] * p[1]),
          theta0 = k * round((theta - halfPi$3) / k) + halfPi$3,
          alpha = atan2$2(sin$2(theta -= theta0), 2 - cos$2(theta)); // angle relative to lobe end
      theta = theta0 + asin$2(pi$4 / r * sin$2(alpha)) - alpha;
      p[0] = r * cos$2(theta);
      p[1] = r * sin$2(theta);
    }
    return p;
  ...
  forward.invert = function(x, y) {
    var r = sqrt$3(x * x + y * y);
    if (r > halfPi$3) {
      var theta = atan2$2(y, x),
          theta0 = k * round((theta - halfPi$3) / k) + halfPi$3,
          s = theta > theta0 ? -1 : 1,
          A = r * cos$2(theta0 - theta),
          cotAlpha = 1 / tan$1(s * acos$2((A - pi$4) / sqrt$3(pi$4 * (pi$4 - 2 * A) + r * r)));
      theta = theta0 + 2 * atan$1((cotAlpha + s * sqrt$3(cotAlpha * cotAlpha - 3)) / 3);
      x = r * cos$2(theta), y = r * sin$2(theta);
    }
...
function boggsRaw(lambda, phi) {
  var theta = mollweideBromleyTheta(pi$4, phi);
  return [k$1 * lambda / (1 / cos$2(phi) + w / cos$2(theta)), (phi + sqrt2 * sin$2(theta)) / k$1];
}

boggsRaw.invert = function(x, y) {
  var ky = k$1 * y, theta = y < 0 ? -quarterPi$1 : quarterPi$1, i = 25, delta, phi;
  do {
...
function bonneRaw(phi0) {
  ...
  function forward(lambda, phi) {
    var rho = cotPhi0 + phi0 - phi,
        e = rho ? lambda * cos$2(phi) / rho : rho;
    return [rho * sin$2(e), cotPhi0 - rho * cos$2(e)];
  }

  forward.invert = function(x, y) {
    var rho = sqrt$3(x * x + (y = cotPhi0 - y) * y),
        phi = cotPhi0 + phi0 - rho;
    return [rho / cos$2(phi) * atan2$2(x, y), phi];
  };
...
function bottomleyRaw(sinPsi) {
  ...
  function forward(lambda, phi) {
    var rho = halfPi$3 - phi,
        eta = rho ? lambda * sinPsi * sin$2(rho) / rho : rho;
    return [rho * sin$2(eta) / sinPsi, halfPi$3 - rho * cos$2(eta)];
  }

  forward.invert = function(x, y) {
    var x1 = x * sinPsi,
        y1 = halfPi$3 - y,
        rho = sqrt$3(x1 * x1 + y1 * y1),
        eta = atan2$2(x1, y1);
    return [(rho ? rho / sin$2(rho) : 1) * eta / sinPsi, halfPi$3 - rho];
  };
...
function distance$1(dPhi, c1, s1, c2, s2, dLambda) {
  var cosdLambda = cos$2(dLambda), r;
...
function longitude(lambda) {
  return lambda - 2 * pi$4 * floor$1((lambda + pi$4) / (2 * pi$4));
}

function chamberlinRaw(p0, p1, p2) {
  var points = [
    [p0[0], p0[1], sin$2(p0[1]), cos$2(p0[1])],
    [p1[0], p1[1], sin$2(p1[1]), cos$2(p1[1])],
    [p2[0], p2[1], sin$2(p2[1]), cos$2(p2[1])]
  ];
  ...
  var mean = [
    points[2].point[0] = points[0].point[0] + points[2].v[0] * cos$2(beta0),
    2 * (points[0].point[1] = points[1].point[1] = points[2].v[0] * sin$2(beta0))
  ];
...
function collignonRaw(lambda, phi) {
  var alpha = sqrt$3(1 - sin$2(phi));
  return [(2 / sqrtPi) * lambda * alpha, sqrtPi * (1 - alpha)];
}

collignonRaw.invert = function(x, y) {
  var lambda = (lambda = y / sqrtPi - 1) * lambda;
  return [lambda > 0 ? x * sqrt$3(pi$4 / lambda) / 2 : 0, asin$2(1 - lambda)];
};
...
var sqrt3$1 = sqrt$3(3);

function crasterRaw(lambda, phi) {
  return [sqrt3$1 * lambda * (2 * cos$2(2 * phi / 3) - 1) / sqrtPi, sqrt3$1 * sqrtPi * sin$2(phi / 3)];
}

crasterRaw.invert = function(x, y) {
  var phi = 3 * asin$2(y / (sqrt3$1 * sqrtPi));
  return [sqrtPi * x / (sqrt3$1 * (2 * cos$2(2 * phi / 3) - 1)), phi];
};
...
function eckert2Raw(lambda, phi) {
  var alpha = sqrt$3(4 - 3 * sin$2(abs$2(phi)));
  return [
    2 / sqrt$3(6 * pi$4) * lambda * alpha,
    sign$2(phi) * sqrt$3(2 * pi$4 / 3) * (2 - alpha)
  ];
}

eckert2Raw.invert = function(x, y) {
  var alpha = 2 - abs$2(y) / sqrt$3(2 * pi$4 / 3);
  return [
    x * sqrt$3(6 * pi$4) / (2 * alpha),
    sign$2(y) * asin$2((4 - alpha * alpha) / 3)
  ];
};
...
function eckert3Raw(lambda, phi) {
  ...
  return [
    2 / k * lambda * (1 + sqrt$3(1 - 4 * phi * phi / (pi$4 * pi$4))),
    4 / k * phi
  ];
}
...
eckert3Raw.invert = function(x, y) {
  ...
  return [
    x * k / (1 + sqrt$3(1 - y * y * (4 + pi$4) / (4 * pi$4))),
    y * k / 2
  ];
};
...
function eckert4Raw(lambda, phi) {
  ...
  return [
    2 / sqrt$3(pi$4 * (4 + pi$4)) * lambda * (1 + cos$2(phi)),
    2 * sqrt$3(pi$4 / (4 + pi$4)) * sin$2(phi)
  ];
}
...
eckert4Raw.invert = function(x, y) {
  var A = y * sqrt$3((4 + pi$4) / pi$4) / 2,
      ...
      c = cos$2(k);
  return [
    x / (2 / sqrt$3(pi$4 * (4 + pi$4)) * (1 + c)),
    asin$2((k + A * (c + 2)) / (2 + halfPi$3))
  ];
};
...
function eckert5Raw(lambda, phi) {
  return [
    lambda * (1 + cos$2(phi)) / sqrt$3(2 + pi$4),
    2 * phi / sqrt$3(2 + pi$4)
  ];
}
...
function eisenlohrRaw(lambda, phi) {
  var s0 = sin$2(lambda /= 2),
      c0 = cos$2(lambda),
      k = sqrt$3(cos$2(phi)),
      c1 = cos$2(phi /= 2),
      t = sin$2(phi) / (c1 + sqrt2 * c0 * k),
      c = sqrt$3(2 / (1 + t * t)),
      v = sqrt$3((sqrt2 * c1 + (c0 + s0) * k) / (sqrt2 * c1 + (c0 - s0) * k));
  return [
    eisenlohrK * (c * (v - 1 / v) - 2 * log$2(v)),
...
function gilbertForward(point) {
  return [point[0] / 2, asin$2(tan$1(point[1] / 2 * radians$1)) * degrees$2];
}
...
function gingeryRaw(rho, n) {
  ...
  function forward(lambda, phi) {
    ...
    if (r2 > rho2) {
      var r = sqrt$3(r2),
          ...
          alpha = theta - theta0,
          rhoCosAlpha = rho * cos$2(alpha),
          k_ = (rho * sin$2(alpha) - alpha * sin$2(rhoCosAlpha)) / (halfPi$3 - rhoCosAlpha),
          s_ = gingeryLength(alpha, k_),
          e = (pi$4 - rho) / gingeryIntegrate(s_, rhoCosAlpha, pi$4);
  ...
  forward.invert = function(x, y) {
    ...
    if (r2 > rho2) {
      ...
      var x_halfPi = x - halfPi$3,
          sinx = sin$2(x),
          alpha = y / sinx,
          delta = x < halfPi$3 ? Infinity : 0,
          i = 10;

      while (true) {
        var rhosinAlpha = rho * sin$2(alpha),
            rhoCosAlpha = rho * cos$2(alpha),
            sinRhoCosAlpha = sin$2(rhoCosAlpha),
            halfPi_RhoCosAlpha = halfPi$3 - rhoCosAlpha,
        ...
        alpha -= delta = (alpha * sinx - k_ * x_halfPi - y) / (
          sinx - x_halfPi * 2 * (
            halfPi_RhoCosAlpha * (rhoCosAlpha + alpha * rhosinAlpha * cos$2(rhoCosAlpha) - sinRhoCosAlpha) -
            rhosinAlpha * (rhosinAlpha - alpha * sinRhoCosAlpha)
          ) / (halfPi_RhoCosAlpha * halfPi_RhoCosAlpha));
      }
      r = rho + gingeryIntegrate(s_, rhoCosAlpha, x) * (pi$4 - rho) / gingeryIntegrate(s_, rhoCosAlpha, pi$4);
      theta = theta0 + alpha;
      x = r * cos$2(theta);
      y = r * sin$2(theta);
    }
...
function gingeryLength(alpha, k) {
  return function(x) {
    var y_ = alpha * cos$2(x);
    if (x < halfPi$3) y_ -= k;
    return sqrt$3(1 + y_ * y_);
  };
}
...
var squareRaw = function(project) {
  var dx = project(halfPi$3, 0)[0] - project(-halfPi$3, 0)[0];

  function projectSquare(lambda, phi) {
    var s = lambda > 0 ? -0.5 : 0.5,
        point = project(lambda + s * pi$4, phi);
    point[0] -= s * dx;
    return point;
  }
  ...
  if (project.invert) projectSquare.invert = function(x, y) {
    var s = x > 0 ? -0.5 : 0.5,
        ...
        lambda = location[0] - s * pi$4;
    if (lambda < -pi$4) lambda += 2 * pi$4;
    else if (lambda > pi$4) lambda -= 2 * pi$4;
    location[0] = lambda;
    return location;
  };
...
gringortenRaw.invert = function(x, y) {
  if (abs$2(x) > 1) x = sign$2(x) * 2 - x;
  if (abs$2(y) > 1) y = sign$2(y) * 2 - y;
  var sx = sign$2(x),
      sy = sign$2(y),
      x0 = -sx * x,
      y0 = -sy * y,
      ...
      phi = p[1],
      cosPhi = cos$2(phi);
  if (t) lambda = -halfPi$3 - lambda;
  return [sx * (atan2$2(sin$2(lambda) * cosPhi, -sin$2(phi)) + pi$4), sy * asin$2(cos$2(lambda) * cosPhi)];
};

function gringortenHexadecant(lambda, phi) {
  if (phi === halfPi$3) return [0, 0];
  ...
  var cosPhi = cos$2(phi),
      ...
      lambda1 = 4 * lambda / pi$4,
      delta;

  // Slower but accurate bisection method.
  if (lambda > 0.222 * pi$4 || phi < pi$4 / 4 && lambda > 0.175 * pi$4) {
    x = (h + r * sqrt$3(a2 * (1 + r2) - h * h)) / (1 + r2);
    if (lambda > pi$4 / 4) return [x, x];
    var x1 = x, x0 = 0.5 * x;
    x = 0.5 * (x0 + x1), i = 50;
    do {
      var g = sqrt$3(a2 - x * x),
          f = (x * (zeta + mu * g) + nu * asin$2(x / a)) - lambda1;
      if (!f) break;
      if (f < 0) x0 = x;
      else x1 = x;
      x = 0.5 * (x0 + x1);
...
function gringortenHexadecantInvert(x, y) {
  ...
  return [pi$4 / 4 * (x * (zeta + mu * g) + nu * asin$2(x / sqrt$3(a2))), phi];
}
...
function ellipticJ(u, m) {
  ...
  if (m >= 1 - epsilon$5) {
    ...
    return [
      ...
      phi - ai * t * phi * (twon - u),
      phi + ai * t * phi * (twon + u),
      2 * atan$1(exp$1(u)) - halfPi$3 + ai * (twon - u) / b
    ];
  }

  var a = [1, 0, 0, 0, 0, 0, 0, 0, 0],
      c = [sqrt$3(m), 0, 0, 0, 0, 0, 0, 0, 0],
      i = 0;
  b = sqrt$3(1 - m);
  twon = 1;
  ...
  phi = twon * a[i] * u;
  do {
    t = c[i] * sin$2(b = phi) / a[i];
    phi = (asin$2(t) + phi) / 2;
  } while (--i);

  return [sin$2(phi), t = cos$2(phi), t / cos$2(phi - b), phi];
}
...
function ellipticFi(phi, psi, m) {
  var r = abs$2(phi),
      ...
      sinhPsi = sinh$1(i);
  if (r) {
    var cscPhi = 1 / sin$2(r),
        cotPhi2 = 1 / (tan$1(r) * tan$1(r)),
        b = -(cotPhi2 + m * (sinhPsi * sinhPsi * cscPhi * cscPhi) - 1 + m),
        c = (m - 1) * cotPhi2,
        cotLambda2 = (-b + sqrt$3(b * b - 4 * c)) / 2;
    return [
      ellipticF(atan$1(1 / sqrt$3(cotLambda2)), m) * sign$2(phi),
      ellipticF(atan$1(sqrt$3((cotLambda2 / cotPhi2 - 1) / m)), 1 - m) * sign$2(psi)
    ];
  }
  return [
    0,
    ellipticF(atan$1(sinhPsi), 1 - m) * sign$2(psi)
  ];
}
...
function ellipticF(phi, m) {
  if (!m) return phi;
  if (m === 1) return log$2(tan$1(phi / 2 + quarterPi$1));
  var a = 1,
      b = sqrt$3(1 - m),
      c = sqrt$3(m);
  for (var i = 0; abs$2(c) > epsilon$5; i++) {
    if (phi % pi$4) {
      ...
      if (dPhi < 0) dPhi += pi$4;
      phi += dPhi + ~~(phi / pi$4) * pi$4;
    } else phi += phi;
    c = (a + b) / 2;
    b = sqrt$3(a * b);
    c = ((a = c) - b) / 2;
  }
  return phi / (pow$2(2, i) * a);
}

function guyouRaw(lambda, phi) {
  var k_ = (sqrt2 - 1) / (sqrt2 + 1),
      k = sqrt$3(1 - k_ * k_),
      K = ellipticF(halfPi$3, k * k),
      f = -1,
      psi = log$2(tan$1(pi$4 / 4 + abs$2(phi) / 2)),
      r = exp$1(f * psi) / sqrt$3(k_),
      at = guyouComplexAtan(r * cos$2(f * lambda), r * sin$2(f * lambda)),
...
function guyouComplexAtan(x, y) {
  var x2 = x * x,
      ...
      t = 1 - x2 - y * y;
  return [
   0.5 * ((x >= 0 ? halfPi$3 : -halfPi$3) - atan2$2(t, 2 * x)),
    -0.25 * log$2(t * t + 4 * x2) +0.5 * log$2(y_1 * y_1 + x2)
  ];
}
...
guyouRaw.invert = function(x, y) {
  var k_ = (sqrt2 - 1) / (sqrt2 + 1),
      ...
      lambda = atan2$2(tn[1], tn[0]) / f;
  return [
    lambda,
    2 * atan$1(exp$1(0.5 / f * log$2(k_ * tn[0] * tn[0] + k_ * tn[1] * tn[1]))) - halfPi$3
  ];
};
...
function hammerRetroazimuthalRaw(phi0) {
  ...
  forward.invert = function(x, y) {
    var rho = sqrt$3(x * x + y * y),
        sinz = -sin$2(rho),
        cosz = cos$2(rho),
        a = rho * cosz,
        b = -y * sinz,
        c = rho * sinPhi0,
        d = sqrt$3(a * a + b * b - c * c),
        phi = atan2$2(a * c + b * d, b * c - a * d),
        lambda = (rho > halfPi$3 ? -1 : 1) * atan2$2(x * sinz, rho * cos$2(phi) * cosz + y * sin$2(phi) * sinz);
...
function hammerRetroazimuthalRotation(phi0) {
  ...
  return function(lambda, phi) {
    var cosPhi = cos$2(phi),
        ...
        z = sin$2(phi);
    return [
      atan2$2(y, x * cosPhi0 - z * sinPhi0),
      asin$2(z * cosPhi0 + x * sinPhi0)
    ];
  };
}
...
function healpixRaw(H) {
  var phi0 = healpixParallel * radians$1,
      dx = collignonRaw(pi$4, phi0)[0] - collignonRaw(-pi$4, phi0)[0],
      y0 = healpixLambert(0, phi0)[1],
      y1 = collignonRaw(0, phi0)[1],
      dy1 = sqrtPi - y1,
      k = tau$4 / H,
      w = 4 / tau$4,
      h = y0 + (dy1 * dy1 * 4) / tau$4;

  function forward(lambda, phi) {
    var point,
        phi2 = abs$2(phi);
    if (phi2 > phi0) {
      var i = min$2(H - 1, max$2(0, floor$1((lambda + pi$4) / k)));
      lambda += pi$4 * (H - 1) / H - i * k;
      point = collignonRaw(lambda, phi2);
      point[0] = point[0] * tau$4 / dx - tau$4 * (H - 1) / (2 * H) + i * tau$4 / H;
      point[1] = y0 + (point[1] - y1) * 4 * dy1 / tau$4;
  ...
  forward.invert = function(x, y) {
    ...
    var y2 = abs$2(y);
    if (y2 > y0) {
      var i = min$2(H - 1, max$2(0, floor$1((x + pi$4) / k)));
      x = (x + pi$4 * (H - 1) / H - i * k) * dx / tau$4;
      var point = collignonRaw.invert(x, 0.25 * (y2 - y0) * tau$4 / dy1 + y1);
      point[0] -= pi$4 * (H - 1) / H - i * k;
...
function hillRaw(K) {
  var L = 1 + K,
      sinBt = sin$2(1 / L),
      Bt = asin$2(sinBt),
      A = 2 * sqrt$3(pi$4 / (B = pi$4 + 4 * Bt * L)),
      B,
      rho0 = 0.5 * A * (L + sqrt$3(K * (2 + K))),
      K2 = K * K,
      L2 = L * L;
  ...
  function forward(lambda, phi) {
    var t = 1 - sin$2(phi),
        ...
        omega;
    if (t && t < 2) {
      var theta = halfPi$3 - phi, i = 25, delta;
      do {
        var sinTheta = sin$2(theta),
            cosTheta = cos$2(theta),
      ...
      rho = A * (K + t);
      omega = lambda * Bt / pi$4;
    }
    return [
      rho * sin$2(omega),
      rho0 - rho * cos$2(omega)
    ];
  }
  ...
  forward.invert = function(x, y) {
    var rho2 = x * x + (y -= rho0) * y,
        ...
        theta = acos$2(cosTheta),
        sinTheta = sin$2(theta),
        Bt_Bt1 = Bt + atan2$2(sinTheta, L - cosTheta);
    return [
      asin$2(x / sqrt$3(rho2)) * pi$4 / Bt_Bt1,
      asin$2(1 - 2 * (theta - K2 * Bt - L * sinTheta + (1 + L2 - 2 * L * cosTheta) * Bt_Bt1) / B)
    ];
  };
...
homolosineRaw.invert = function(x, y) {
  return abs$2(y) > sinuMollweidePhi
...
var boggs$1 = function() {
  return interrupt$1(boggsRaw, lobes)
      .scale(160.857);
};
...
var homolosine$1 = function() {
  return interrupt$1(homolosineRaw, lobes$1)
      .scale(152.63);
};
...
var sinuMollweide$1 = function() {
  return interrupt$1(sinuMollweideRaw, lobes$4)
      .rotate([-20, -55])
      .scale(164.263)
      .center([0, -5.4036]);
};
...
var sinusoidal$1 = function() {
  return interrupt$1(sinusoidalRaw, lobes$5)
      .scale(152.63)
      .rotate([-20, 0]);
};
...
function modifiedStereographicRaw(C) {
  ...
  function forward(lambda, phi) {
    var cosPhi = cos$2(phi),
        k = 2 / (1 + cosPhi * cos$2(lambda)),
        zr = k * cosPhi * sin$2(lambda),
        zi = k * sin$2(phi),
        i = m,
        w = C[i],
        ar = w[0],
  ...
  forward.invert = function(x, y) {
    ...
    if (i) {
      var rho = sqrt$3(zr * zr + zi * zi),
          c = 2 * atan$1(rho * 0.5),
          sinc = sin$2(c);
      return [atan2$2(zr * sinc, rho * cos$2(c)), rho ? asin$2(zi * sinc / rho) : 0];
    }
  };
...
function length$3(v) {
  return sqrt$3(v[0] * v[0] + v[1] * v[1]);
}

// Angle between two 2D vectors.
function angle$2(a, b) {
  return atan2$2(a[0] * b[1] - a[1] * b[0], a[0] * b[0] + a[1] * b[1]);
}
...
var polyhedral = function(root, face, r) {
  ...
  r = r == null ? -pi$4 / 6 : r; // TODO automate
  ...
  function forward(lambda, phi) {
    var node = face(lambda, phi),
        point = node.project([lambda * degrees$2, phi * degrees$2]),
        t;
    if (t = node.transform) {
      return [
...
collignonK.invert = function(x,y) {
  return collignonRaw.invert(x / kx$1, y);
};
...
var contains$2 = function(ring, point) {
  var x = point[0],
      y = point[1],
      contains = false;
...
var quincuncial = function(project) {
  ...
  function projectQuincuncial(lambda, phi) {
    var t = abs$2(lambda) < halfPi$3,
        p = project(t ? lambda : lambda > 0 ? lambda - pi$4 : lambda + pi$4, phi),
        x = (p[0] - p[1]) * sqrt1_2,
        y = (p[0] + p[1]) * sqrt1_2;
    if (t) return [x, y];
    var d = dx * sqrt1_2,
        s = x > 0 ^ y > 0 ? -1 : 1;
    return [s * x - sign$2(y) * d, s * y - sign$2(x) * d];
  }

  if (project.invert) projectQuincuncial.invert = function(x0, y0) {
    var x = (x0 + y0) * sqrt1_2,
        y = (y0 - x0) * sqrt1_2,
        t = abs$2(x) < 0.5 * dx && abs$2(y) < 0.5 * dx;
...
function robinsonRaw(lambda, phi) {
  ...
  return [
    ...
    (phi > 0 ? halfPi$3 : -halfPi$3) * (by + di * (cy - ay) / 2 + di * di * (cy - 2 * by + ay) / 2)
  ];
}
...
var A$3 = 4 * pi$4 + 3 * sqrt$3(3);
var B$3 = 2 * sqrt$3(2 * pi$4 * sqrt$3(3) / A$3);

var wagner4Raw = mollweideBromleyRaw(B$3 * sqrt$3(3) / pi$4, B$3, A$3 / 6);
